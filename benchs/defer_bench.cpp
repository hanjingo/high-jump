...
static void bm_make_defer_factory(benchmark::State &state)
{
    int counter = 0;
    for(auto _ : state)
    {
        auto defer_obj = hj::make_defer([&counter]() { counter++; });
        benchmark::DoNotOptimize(counter);
        benchmark::DoNotOptimize(defer_obj);
    }
}
BENCHMARK(bm_make_defer_factory);

static void bm_defer_conditional_true(benchmark::State &state)
{
    int counter = 0;
    for(auto _ : state)
    {
        DEFER_IF(true, counter++;);
        benchmark::DoNotOptimize(counter);
    }
}
BENCHMARK(bm_defer_conditional_true);

static void bm_defer_conditional_false(benchmark::State &state)
{
    int counter = 0;
    for(auto _ : state)
    {
        DEFER_IF(false, counter++;);
        benchmark::DoNotOptimize(counter);
    }
}
BENCHMARK(bm_defer_conditional_false);

static void bm_defer_manual_cancel(benchmark::State &state)
{
    int counter = 0;
    for(auto _ : state)
    {
...
BENCHMARK(bm_defer_manual_cancel);

static void bm_defer_manual_execute(benchmark::State &state)
{
    int counter = 0;
    for(auto _ : state)
    {
...
BENCHMARK(bm_defer_manual_execute);

static void bm_defer_move_semantics(benchmark::State &state)
{
    int counter = 0;
    for(auto _ : state)
    {
        auto defer1 = hj::make_defer([&counter]() { counter++; });
        auto defer2 = std::move(defer1);
        benchmark::DoNotOptimize(counter);
        benchmark::DoNotOptimize(defer2);
    }
}
BENCHMARK(bm_defer_move_semantics);
