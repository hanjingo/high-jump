                   ...
                   bool succeeded)
{
    ...
    std::copy(w_minidump_id.begin(),
              w_minidump_id.end(),
              str_minidump_id.begin());

    std::cout << (succeeded ? "Succeed" : "Fail")
              << " To Create Dump File With Path =" << str_dump_dir << "/"
              << str_minidump_id << std::endl;
    return true;
}
#elif __APPLE__
...
{
    std::cout << (succeeded ? "Succeed" : "Fail")
              << " To Create Dump File With Path =" << std::string(dump_dir)
              << "/" << std::string(minidump_id) << std::endl;
    return true;
}
#else
bool dump_callback(const google_breakpad::MinidumpDescriptor& descriptor,
                   ...
                   bool succeeded)
{
    std::cout << (succeeded ? "Succeed" : "Fail") << " To Create Dump File"
              << std::endl;
    return true;
}
#endif
