...
int main(int argc, char* argv[])
{
    std::thread([]() {
        ...
        srv.Get("/hello", [](const libcpp::http_request & req, libcpp::http_response & resp) {
            std::cout << "On GET /hello" << std::endl;
            resp.set_content("world", "text/plain");
        });
        srv.Post("/person",
        [](const libcpp::http_request & req, libcpp::http_response & resp, const libcpp::http_content_reader & reader) {
            std::cout << "On POST /person" << std::endl;

            libcpp::http_multipart_form_data_items items;
            if (req.is_multipart_form_data()) {
                reader([&](const libcpp::http_multipart_form_data & item) {
                ...
                std::cout << "text1=" << items.at(0).content << std::endl;
            } else {
                std::string body;
    ...
    std::thread([]() {
        ...
        std::cout << "http cli GET http://localhost:10086/hello with >>" << std::endl;
        ...
        std::cout << "http cli POST http://localhost:10086/person with with http_params>>" << std::endl;
        std::cout << "\n" << std::endl;
        ...
        std::cout << "http cli POST http://localhost:10086/person with with MultipartFormDataItems>>" << std::endl;
        std::cout << "\n" << std::endl;
    }).detach();
