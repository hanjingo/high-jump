...
int main(int argc, char *argv[])
{
    ...
    std::thread t1([]() {
        ...
        std::cout << "Srv>> listen addr "
                  << inet_ntop(AF_INET, &srv_addr.sin_addr, buf, sizeof(buf))
        ...
        for (;;)
        {
            ...
            for (int i = 0; i < nfds; ++i)
            {
                ...
                else if (events[i].events & EPOLLIN)  // data received
                {
                    ...
                    std::cout << "Srv>> recv data=" << std::string(rbuf)
                              << std::endl;
                }
                ...
                else if (events[i].events & EPOLLERR)  // error occur, close it
                {
                    ...
                    close(events[i].data.fd);
                         ...
                         EPOLLHUP)  // sockets be hupped, close it
                {
