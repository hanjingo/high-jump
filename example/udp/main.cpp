...
int main(int argc, char* argv[])
{
    ...
    std::thread t2([]() {
        ...
        libcpp::udp_socket::endpoint_t ep =
            libcpp::udp_socket::endpoint_v4(10086);
    ...
    std::thread t3([]() {
        ...
        sock.async_send(
            ...
            5,
            ep,
            [&](const libcpp::udp_socket::err_t& err, std::size_t len) {
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
                std::cout << "Cli> asyc send " << msg << " with len=" << len
                          << std::endl;
            });
        ...
        sock.async_recv(
            ...
            [&](const libcpp::udp_socket::err_t& err, std::size_t len) {
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
                std::cout << "Cli> async recv " << std::string(buf)
