...
int main(int argc, char* argv[])
{
    ...
    for (libcpp::xml_node_iterator itr = childs.begin(); itr != childs.end();
         itr++)
    {
        std::cout << "find child>> " << itr->name() << std::endl;
        if (itr->name() == std::string("man"))
        {
            std::cout << "name: " << itr->attribute("name").as_string()
                      << std::endl;
            std::cout << "age: " << itr->attribute("age").as_int() << std::endl;
        }
    }
    auto attr = doc.child("man").find_attribute(
        [](const libcpp::xml_attribute& arg) -> bool {
