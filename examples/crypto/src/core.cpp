...
crypto_core::crypto_core()
{
    auto err = load();
    if(err)
        throw std::runtime_error("crypto_core load fail with err: "
                                 + err.message());
}
...
err_t crypto_core::init(const size_t data_pool_size)
{
    if(_init == nullptr)
        return error(CRYPTO_ERR_CORE_NOT_LOAD);
...
err_t crypto_core::encrypt(std::string       &out,
                           const std::string &in,
...
err_t crypto_core::decrypt(std::string       &out,
                           const std::string &in,
                           const std::string &content,
                           const std::string &algo,
                           const std::string &mode,
...
err_t crypto_core::keygen(std::vector<std::string> &outs,
                          const std::string        &algo,
                          const std::string        &fmt,
                          const std::string        &mode,
                          const size_t              bits)
...
db_core::db_core()
{
    auto err = load();
    if(err)
        throw std::runtime_error("db_core load fail with err: "
                                 + err.message());
}
...
err_t db_core::exec(const std::string &db_id, const std::string &sql)
{
    if(_exec == nullptr)
        return error(DB_ERR_CORE_NOT_LOAD);
...
err_t db_core::add(const std::string &db_id, const std::string &key)
{
                     ...
                     const int                               count)
{
    ...
    if(db_id == "dict")
    {
        ...
        sql = std::string("SELECT " + fmt_strs(contents) + " FROM " + tbl
                          + limit_sql + ";");
