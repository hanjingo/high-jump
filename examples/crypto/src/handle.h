...
static void handle_err(std::error_code err, Args&& ... args)
{
    switch (err.value())
    {
        ...
        case FAIL:
        {
            LOG_ERROR(tr("log.fail").c_str(), std::forward<Args>(args)...);
            break;
        }
        case ERR_CORE_NOT_LOAD:
        {
            LOG_ERROR(tr("log.err_core_not_load").c_str(), std::forward<Args>(args)...);
            break;
        }
        case ERR_CORE_LOAD_FAIL:
        {
            LOG_ERROR(tr("log.err_core_load_fail").c_str(), std::forward<Args>(args)...);
            break;
        }
        case ERR_VERSION_MISMATCH:
        {
            LOG_ERROR(tr("log.err_version_mismatch").c_str(), std::forward<Args>(args)...);
            break;
        }
        case ERR_INIT_FAIL:
        {
            LOG_ERROR(tr("log.err_init_fail").c_str(), std::forward<Args>(args)...);
            break;
        }
        case ERR_QUIT_FAIL:
        {
            LOG_ERROR(tr("log.err_quit_fail").c_str(), std::forward<Args>(args)...);
            break;
        }

        case ERR_INVALID_SUBCMD: 
        {
            LOG_ERROR(tr("log.err_invalid_subcmd").c_str(), 
                std::forward<Args>(args)..., 
                fmt_strs(std::vector<std::string>{"encrypt", "decrypt", "keygen", "guess", "dict", "help"}));
            break;
        }
        case ERR_INVALID_FMT: 
        {
            LOG_ERROR(tr("log.err_invalid_fmt").c_str(), 
                std::forward<Args>(args)..., 
        ...
        {
            LOG_ERROR(tr("log.err_invalid_algo").c_str(), 
                std::forward<Args>(args)..., 
        ...
        {
            LOG_ERROR(tr("log.err_invalid_mode").c_str(), 
                std::forward<Args>(args)..., 
        ...
        {
            LOG_ERROR(tr("log.err_format_fail").c_str(), std::forward<Args>(args)...);
            break;
        }
        case ERR_FORMAT_STYLE_NOT_FOUND:
        {
            LOG_ERROR(tr("log.err_format_style_not_found").c_str(), std::forward<Args>(args)...);
            break;
        }
        case ERR_FORMAT_INVALID_TARGET:
        {
            LOG_ERROR(tr("log.err_format_invalid_target").c_str(), std::forward<Args>(args)...);
            break;
        }
        case ERR_FORMAT_HEX_FAILED:
        {
            LOG_ERROR(tr("log.err_format_hex_failed").c_str(), std::forward<Args>(args)...);
            break;
        }
        case ERR_FORMAT_BASE64_FAILED:
        {
            LOG_ERROR(tr("log.err_format_base64_failed").c_str(), std::forward<Args>(args)...);
            break;
        }

        case ERR_UNFORMAT_FAIL:
        {
            LOG_ERROR(tr("log.err_unformat_fail").c_str(), std::forward<Args>(args)...);
            break;
        }
        case ERR_UNFORMAT_STYLE_NOT_FOUND:
        {
            LOG_ERROR(tr("log.err_unformat_style_not_found").c_str(), std::forward<Args>(args)...);
            break;
        }
        case ERR_UNFORMAT_INVALID_TARGET:
        {
            LOG_ERROR(tr("log.err_unformat_invalid_target").c_str(), std::forward<Args>(args)...);
            break;
        }
        case ERR_UNFORMAT_HEX_FAILED:
        {
            LOG_ERROR(tr("log.err_unformat_hex_failed").c_str(), std::forward<Args>(args)...);
            break;
        }
        case ERR_UNFORMAT_BASE64_FAILED:
        {
            LOG_ERROR(tr("log.err_unformat_base64_failed").c_str(), std::forward<Args>(args)...);
            break;
        }
