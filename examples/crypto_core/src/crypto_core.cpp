...
C_STYLE_EXPORT void crypto_encrypt(sdk_context ctx)
{
    ...
    auto content =
        (ret->content == NULL) ? std::string() : std::string(ret->content);
    ...
    auto padding =
        (ret->padding == NULL) ? std::string() : std::string(ret->padding);
    auto iv  = (ret->iv == NULL) ? std::string() : std::string(ret->iv);
    auto fmt = (ret->fmt == NULL) ? std::string() : std::string(ret->fmt);
    ret->result =
        encryptor_mgr::instance()
            .encrypt(out, in, content, algo, mode, key, padding, iv, fmt);
...
C_STYLE_EXPORT void crypto_decrypt(sdk_context ctx)
{
    ...
    auto key  = (ret->key == NULL) ? std::string() : std::string(ret->key);
    auto passwd =
        (ret->passwd == NULL) ? std::string() : std::string(ret->passwd);
    auto padding =
        (ret->padding == NULL) ? std::string() : std::string(ret->padding);
    auto iv     = (ret->iv == NULL) ? std::string() : std::string(ret->iv);
    auto fmt    = (ret->fmt == NULL) ? std::string() : std::string(ret->fmt);
    ret->result = decryptor_mgr::instance().decrypt(out,
                                                    in,
                                                    content,
                                                    algo,
                                                    mode,
