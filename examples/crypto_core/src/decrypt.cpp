...
int decryptor_mgr::decrypt(std::string       &out,
                           const std::string &in,
                           ...
                           const std::string &fmt)
{
    for(const auto &e : _decryptors)
    {
        ...
        return e
            ->decrypt(out, in, content, mode, key, passwd, padding, iv, fmt);
    }
...
// ------------------------------------ AES -------------------------------------
int aes_decryptor::decrypt(std::string       &out,
                           const std::string &in,
                           const std::string &content,
                           const std::string &mode,
                           const std::string &key,
...
bool aes_decryptor::_is_mode_valid(const std::string &mode)
{
    ...
    return std::find(std::begin(modes), std::end(modes), mode)
           != std::end(modes);
}

bool aes_decryptor::_is_key_valid(const std::string &key,
                                  const std::string &mode)
{
    auto mod = str_to_aes_mode(mode);
...
bool aes_decryptor::_is_padding_valid(const std::string &padding)
{
    ...
    return std::find(std::begin(paddings), std::end(paddings), padding)
           != std::end(paddings);
}

bool aes_decryptor::_is_iv_valid(const std::string &iv, const std::string &mode)
{
...
// ------------------------------------ BASE64 -------------------------------------
int base64_decryptor::decrypt(std::string       &out,
                              const std::string &in,
                              const std::string &content,
                              const std::string &mode,
                              const std::string &key,
                              ...
                              const std::string &fmt)
{
    ...
    if(out == "" && in == "") // decrypt: mem -> mem
    {
        err = hj::base64::decode(out, content) == hj::base64::error_code::ok
                  ? OK
                  : CRYPTO_ERR_DECRYPT_BASE64_FAILED;
    } else if(out != "" && in != "") // decrypt: file -> file
    {
        err = hj::base64::decode_file(out, in) == hj::base64::error_code::ok
                  ? OK
                  : CRYPTO_ERR_DECRYPT_BASE64_FAILED;
    } else if(out != "" && in == "") // decrypt: mem -> file
    {
        ...
        err = static_cast<int>(hj::base64::decode(fout, sin));
    } else // decrypt: file -> mem
    {
        std::ifstream      fin(in, std::ios::binary);
        std::ostringstream sout(out);
        err = hj::base64::decode(sout, fin) == hj::base64::error_code::ok
                  ? OK
                  : CRYPTO_ERR_DECRYPT_BASE64_FAILED;
        out = sout.str();
    }
...
// ------------------------------------ DES -------------------------------------
int des_decryptor::decrypt(std::string       &out,
                           const std::string &in,
                           const std::string &content,
                           const std::string &mode,
                           const std::string &key,
...
bool des_decryptor::_is_key_valid(const std::string &key,
                                  const std::string &mode)
{
...
bool des_decryptor::_is_iv_valid(const std::string &iv, const std::string &mode)
{
    const unsigned char *iv_ptr =
        (iv == "") ? nullptr
                   : reinterpret_cast<const unsigned char *>(iv.c_str());
...
// ------------------------------------ RSA -------------------------------------
int rsa_decryptor::decrypt(std::string       &out,
                           const std::string &in,
                           const std::string &content,
                           const std::string &mode,
                           const std::string &key,
...
bool rsa_decryptor::_is_input_valid(const std::string &out,
                                    const std::string &in,
                                    const std::string &content,
                                    const std::string &key,
                                    const std::string &padding)
