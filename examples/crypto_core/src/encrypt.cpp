    ...
    const std::string& mode)
{
    ...
    return libcpp::aes::is_iv_valid(str_to_aes_mode(mode), iv_ptr, iv.size());
}
    ...
    const std::string& padding)
{
    ...
    // to mem
    if (out == "")
        return libcpp::aes::is_plain_valid(mod, key.size(), pad, content.size());
    ...
    const std::string& padding)
{
    ...
    // to mem
    if (out == "")
        return libcpp::des::is_plain_valid(reinterpret_cast<const unsigned char*>(content.c_str()), 
            content.size(), pad);
...
bool des_encryptor::_is_key_valid(
    ...
    const std::string& mode)
{
    return libcpp::des::is_key_valid(reinterpret_cast<const unsigned char*>(key.c_str()), key.size());
}
