...
C_STYLE_EXPORT void lic_add_issuer(sdk_context *ctx)
{
    ...
    for(int i = 0; i < ret->keys_num && i < LIC_MAX_KEY_NUM; i++)
    {
...
C_STYLE_EXPORT void lic_issue(sdk_context *ctx)
{
    ...
    for(int i = 0;
        ret->claims_num > 0 && i + 1 < ret->claims_num && i < LIC_MAX_CLAIM_NUM;
        i += 2)
    {
        if(ret->claims[i] == nullptr || ret->claims[i + 1] == nullptr)
            continue;

        claims.emplace_back(std::make_pair(std::string(ret->claims[i]),
                                           std::string(ret->claims[i + 1])));
    }
    ...
    ret->result =
        issuer_mgr::instance().issue(output, licensee, issuer_id, time, claims);
