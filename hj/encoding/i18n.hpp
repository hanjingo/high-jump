...
class i18n
{
  public:
    class translator
    {
      public:
        translator() = default;
        explicit translator(const std::string &locale)
            : _locale_name(locale)
        {
        }

        inline const std::string &locale() const { return _locale_name; }
        inline void               set_locale(const std::string &locale)
        {
            _locale_name = locale;
        }
      ...
      private:
                       ...
                       T                            &&arg) const
        {
            ...
            icu::UnicodeString unicode_str =
                icu::UnicodeString::fromUTF8(std::string(arg));
        ...
        template <typename... Args>
        std::string _format_message(const std::string &pattern,
                                    Args... args) const
        {
            UErrorCode         status = U_ZERO_ERROR;
            icu::Locale        loc(_locale_name.empty() ? "en_US"
                                                 : _locale_name.c_str());
            icu::UnicodeString unicode_pattern =
        ...
        bool
        _load_from_resource_bundle_recursive(const icu::ResourceBundle &bundle,
  ...
  public:
    ...
    bool load_translations_from_directory(
        ...
        const std::string &base_name = "translations")
    {
            ...
            std::filesystem::directory_iterator(directory_path))
        {
            ...
            std::string locale =
                filename.substr(locale_start, locale_end - locale_start);
