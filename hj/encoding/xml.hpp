...
public:
    ...
    xml() : _doc(std::make_shared<pugi::xml_document>()), _node(*_doc) {}
    xml(const xml& rhs) : _doc(rhs._doc), _node(rhs._node) {}
    ~xml() = default;

    inline xml child(const char* name) const { return xml(_doc, _node.child(name)); }
    inline xml append_child(const char* name) { return xml(_doc, _node.append_child(name)); }
    inline bool remove_child(const char* name) { return _node.remove_child(name); }
    inline std::string child_value(const char* name) const { return _node.child_value(name); }
    ...
    inline std::string attr(const char* name) const { return _node.attribute(name).value(); }
    inline void set_attr(const char* name, const char* value) { _node.append_attribute(name).set_value(value); }

    bool load(
        const char* text,
        const parse_options parse = parse_options::by_default) 
    {
        pugi::xml_parse_result r = _doc->load_string(text, static_cast<unsigned int>(parse));
    ...
    {
        pugi::xml_parse_result r = _doc->load(
            in, 
    ...
    bool load_file(
        ...
        const encoding encoding = encoding::by_auto) 
    {
        pugi::xml_parse_result r = _doc->load_file(
            filepath, 
            static_cast<unsigned int>(parse),
            static_cast<pugi::xml_encoding>(encoding));
