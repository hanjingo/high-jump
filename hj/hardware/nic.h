...
inline nic_error_t nic_init(void)
{
...
inline nic_error_t nic_get_interface_count(uint32_t *count)
{
    if(!count)
...
inline nic_error_t nic_enumerate_interfaces(nic_info_t *interfaces,
    ...
    *actual_count = count;
    ...
    while(ifa && count < max_interfaces)
    {
        if(ifa->ifa_addr)
        {
            ...
            nic->status =
                (ifa->ifa_flags & IFF_UP) ? NIC_STATUS_UP : NIC_STATUS_DOWN;
...
inline nic_error_t nic_get_interface_info(const char *interface_name,
                                          nic_info_t *info)
{
    ...
    uint32_t    count;
    nic_error_t result =
        nic_enumerate_interfaces(interfaces, NIC_MAX_INTERFACES, &count);
...
inline nic_error_t nic_get_statistics(const char       *interface_name,
...
inline nic_error_t nic_enable_interface(const char *interface_name)
{
    if(!interface_name)
        return NIC_ERROR_INVALID_PARAMETER;
...
inline nic_error_t nic_disable_interface(const char *interface_name)
{
    if(!interface_name)
        return NIC_ERROR_INVALID_PARAMETER;
