...
{
    RAM_SUCCESS = 0,
    RAM_ERR_INVALID_PARAMETER = -1,
    RAM_ERR_ACCESS_DENIED = -2,
    RAM_ERR_NOT_FOUND = -3,
    RAM_ERR_INSUFFICIENT_BUFFER = -4,
    RAM_ERR_SYSTEM_ERROR = -5,
    RAM_ERR_NOT_SUPPORTED = -6,
    RAM_ERR_INSUFFICIENT_MEMORY = -7,
    RAM_ERR_ALLOCATION_FAILED = -8,
    RAM_ERR_INVALID_ADDRESS = -9,
...
static inline ram_err_t ram_init(void) 
{
 ...
 */
static inline ram_err_t ram_get_system_info(ram_system_info_t* info) 
{
    if (!info) 
 ...
 * @return RAM_SUCCESS on success, error code on failure
 */
static inline ram_err_t ram_get_modules(
    ram_module_info_t* modules, 
 ...
 * @return RAM_SUCCESS on success, error code on failure
 */
static inline ram_err_t ram_allocate_aligned(size_t size, size_t alignment, void** ptr) 
{
 ...
 * @return RAM_SUCCESS on success, error code on failure
 */
static inline ram_err_t ram_free_aligned(void* ptr) 
{
    if (!ptr) 
        return RAM_ERR_INVALID_PARAMETER;
 ...
 * @return RAM_SUCCESS on success, error code on failure
 */
static inline ram_err_t ram_allocate_large_pages(size_t size, void** ptr) {
    if (!ptr || size == 0)
        return RAM_ERR_INVALID_PARAMETER;
 ...
 * @return RAM_SUCCESS on success, error code on failure
 */
static inline ram_err_t ram_free_large_pages(void* ptr, size_t size) {
    if (!ptr || size == 0)
        return RAM_ERR_INVALID_PARAMETER;
 ...
 * @return RAM_SUCCESS on success, error code on failure
 */
static inline ram_err_t ram_protect_memory(void* ptr, size_t size, ram_protection_t protection) 
{
    if (!ptr || size == 0)
        return RAM_ERR_INVALID_PARAMETER;
    ...
    switch (protection) 
    {
        ...
        default: return RAM_ERR_INVALID_PARAMETER;
    }
    ...
    if (protection & RAM_PROTECTION_READ) 
        posix_protection |= PROT_READ;

    if (protection & RAM_PROTECTION_WRITE) 
        posix_protection |= PROT_WRITE;

    if (protection & RAM_PROTECTION_EXECUTE) 
 ...
 * @return RAM_SUCCESS on success, error code on failure
 */
static inline ram_err_t ram_lock_memory(void* ptr, size_t size) 
{
    if (!ptr || size == 0)
        return RAM_ERR_INVALID_PARAMETER;
 ...
 * @return RAM_SUCCESS on success, error code on failure
 */
static inline ram_err_t ram_unlock_memory(void* ptr, size_t size) 
{
    if (!ptr || size == 0)
        return RAM_ERR_INVALID_PARAMETER;
 ...
 * @return RAM_SUCCESS on success, error code on failure
 */
static inline ram_err_t ram_prefetch_memory(const void* ptr, size_t size) 
{
    if (!ptr || size == 0)
        return RAM_ERR_INVALID_PARAMETER;
 ...
 * @return RAM_SUCCESS on success, error code on failure
 */
static inline ram_err_t ram_format_size(
    uint64_t bytes, 
    char* buffer, 
    size_t buffer_size) 
{
 ...
 * @return RAM_SUCCESS on success, error code on failure
 */
static inline ram_err_t ram_get_page_size(uint32_t* page_size) 
{
    if (!page_size)
        return RAM_ERR_INVALID_PARAMETER;
 ...
 * @return RAM_SUCCESS on success, error code on failure
 */
static inline ram_err_t ram_is_valid_address(
    const void* ptr,
    size_t size,
 ...
 * @return RAM_SUCCESS on success, error code on failure
 */
static inline ram_err_t ram_get_process_usage(ram_statistics_t* stats) 
{
    if (!stats) 
        return RAM_ERR_INVALID_PARAMETER;
