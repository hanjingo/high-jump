...
// for qt environment
    ...
    static void default_qt_msg_handler(QtMsgType typ, const QMessageLogContext& context, const QString& str) 
    {
        switch (typ)
        {
            case QtDebugMsg: { hj::logger::instance()->debug(str.toUtf8().constData()); break; }
            case QtWarningMsg: { hj::logger::instance()->warn(str.toUtf8().constData()); break; }
            case QtCriticalMsg: { hj::logger::instance()->critical(str.toUtf8().constData()); break; }
            case QtInfoMsg: { hj::logger::instance()->info(str.toUtf8().constData()); break; }
            default: break;
        };
    };
...
#define LOG_TRACE(...) \
    hj::logger::instance()->trace(__VA_ARGS__)

#define LOG_DEBUG(...) \
    hj::logger::instance()->debug(__VA_ARGS__)

#define LOG_INFO(...) \
    hj::logger::instance()->info(__VA_ARGS__)

#define LOG_WARN(...) \
    hj::logger::instance()->warn(__VA_ARGS__)

#define LOG_ERROR(...) \
    hj::logger::instance()->error(__VA_ARGS__)

#define LOG_CRITICAL(...) \
    hj::logger::instance()->critical(__VA_ARGS__)
