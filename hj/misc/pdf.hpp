...
struct rectangle
{
    ...
    rectangle(float x_pos = 0.0f, float y_pos = 0.0f, float w = 0.0f, float h = 0.0f) 
...
public:
    ...
    bool set_size(size sz, page_orientation orientation = page_orientation::portrait) 
    {
        HPDF_PageSizes hpdf_size;
        
        switch (sz) 
        {
            case size::a4: hpdf_size = HPDF_PAGE_SIZE_A4; break;
            case size::a3: hpdf_size = HPDF_PAGE_SIZE_A3; break;
            case size::a5: hpdf_size = HPDF_PAGE_SIZE_A5; break;
            case size::letter: hpdf_size = HPDF_PAGE_SIZE_LETTER; break;
            case size::legal: hpdf_size = HPDF_PAGE_SIZE_LEGAL; break;
            default: hpdf_size = HPDF_PAGE_SIZE_A4; break;
        }

        HPDF_PageDirection hpdf_direction = (orientation == page_orientation::portrait)
            ? HPDF_PAGE_PORTRAIT : HPDF_PAGE_LANDSCAPE;

        return HPDF_Page_SetSize(page_, hpdf_size, hpdf_direction) == HPDF_OK;
    }
    ...
    bool begin_text() 
    {
        return HPDF_Page_BeginText(page_) == HPDF_OK;
    }

    bool end_text() 
    {
        return HPDF_Page_EndText(page_) == HPDF_OK;
    }
    ...
    bool set_font(font_name font, float size) 
    {
        ...
        switch (font) 
        {
            case font_name::helvetica: font_name = HPDF_Font_GetFontName(HPDF_GetFont(doc_, "Helvetica", NULL)); break;
            case font_name::helvetica_bold: font_name = HPDF_Font_GetFontName(HPDF_GetFont(doc_, "Helvetica-Bold", NULL)); break;
            case font_name::helvetica_oblique: font_name = HPDF_Font_GetFontName(HPDF_GetFont(doc_, "Helvetica-Oblique", NULL)); break;
            case font_name::helvetica_bold_oblique: font_name = HPDF_Font_GetFontName(HPDF_GetFont(doc_, "Helvetica-BoldOblique", NULL)); break;
            case font_name::times_roman: font_name = HPDF_Font_GetFontName(HPDF_GetFont(doc_, "Times-Roman", NULL)); break;
            case font_name::times_bold: font_name = HPDF_Font_GetFontName(HPDF_GetFont(doc_, "Times-Bold", NULL)); break;
            case font_name::times_italic: font_name = HPDF_Font_GetFontName(HPDF_GetFont(doc_, "Times-Italic", NULL)); break;
            case font_name::times_bold_italic: font_name = HPDF_Font_GetFontName(HPDF_GetFont(doc_, "Times-BoldItalic", NULL)); break;
            case font_name::courier: font_name = HPDF_Font_GetFontName(HPDF_GetFont(doc_, "Courier", NULL)); break;
            case font_name::courier_bold: font_name = HPDF_Font_GetFontName(HPDF_GetFont(doc_, "Courier-Bold", NULL)); break;
            case font_name::courier_oblique: font_name = HPDF_Font_GetFontName(HPDF_GetFont(doc_, "Courier-Oblique", NULL)); break;
            case font_name::courier_bold_oblique: font_name = HPDF_Font_GetFontName(HPDF_GetFont(doc_, "Courier-BoldOblique", NULL)); break;
            default: font_name = "Helvetica"; break;
        }
    ...
    {
        return HPDF_Page_SetRGBFill(page_, color.r, color.g, color.b) == HPDF_OK;
    }

    bool set_stroke_color(const color& color) 
    {
        return HPDF_Page_SetRGBStroke(page_, color.r, color.g, color.b) == HPDF_OK;
    }
    ...
    bool close_path() 
    {
        return HPDF_Page_ClosePath(page_) == HPDF_OK;
    }
    ...
    {
        return HPDF_Page_TextOut(page_, point.x, point.y, text.c_str()) == HPDF_OK;
    }
    ...
    bool draw_ellipse(const point& center, float x_radius, float y_radius, bool fill = false) 
    {
    ...
    {
        return HPDF_Page_TextOut(page_, position.x, position.y, text.c_str()) == HPDF_OK;
    }
    ...
    {
        HPDF_Image image = HPDF_LoadPngImageFromFile(doc_, filename.c_str());
        if (!image)
            image = HPDF_LoadJpegImageFromFile(doc_, filename.c_str());
        
        if (image)
            HPDF_Page_DrawImage(page_, image, rect.x, rect.y, rect.width, rect.height);
        else
            return false;
    ...
    bool draw_image(const std::string& filename, const point& pos, const float width, const float height) 
    {
        HPDF_Image image = HPDF_LoadPngImageFromFile(doc_, filename.c_str());
        if (!image)
            image = HPDF_LoadJpegImageFromFile(doc_, filename.c_str());
    ...
    bool insert_image(const std::string& filename, const rectangle& rectangle) 
    {
        HPDF_Image image = HPDF_LoadPngImageFromFile(doc_, filename.c_str());
        if (!image)
            image = HPDF_LoadJpegImageFromFile(doc_, filename.c_str());
        ...
        if (image)
            HPDF_Page_DrawImage(page_, image, rectangle.x, rectangle.y, rectangle.width, rectangle.height);
        else
            return false;
    ...
    bool insert_image(const std::string& filename, const point& pos, const float width, const float height) 
    {
        HPDF_Image image = HPDF_LoadPngImageFromFile(doc_, filename.c_str());
        if (!image)
            image = HPDF_LoadJpegImageFromFile(doc_, filename.c_str());
...
public:
    enum permission : HPDF_UINT
    {
        ...
        all = HPDF_ENABLE_PRINT | HPDF_ENABLE_EDIT_ALL | HPDF_ENABLE_COPY | HPDF_ENABLE_EDIT
    };

public:
    document() 
        : doc_(nullptr) 
        , perm_(permission::all)
    {
    ...
    page& add_page() 
    {
        ...
        auto pdf_page = std::make_unique<hj::pdf::page>(doc_, origin);
    ...
    void set_password(const char* owner_passwd, const char* user_passwd = "") 
    {
    ...
    void clear_all_permissions() 
    {
        set_permission(permission::none);
    }
...
private:
    ...
    {
        throw exception("HPDF Error: " + std::to_string(error_no) + ", Detail: " + std::to_string(detail_no));
    }
