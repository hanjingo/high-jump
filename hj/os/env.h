...
inline int64_t env_get(conf_t conf)
{
#if defined(_WIN32) || defined(_WIN64)
    ...
    switch(conf)
    {
        ...
        case CONF_NETWORK_INTERFACES: {
            ...
            if(pAdapterInfo
               && GetAdaptersInfo(pAdapterInfo, &dwSize) == NO_ERROR)
            {
                int              count    = 0;
        ...
        case CONF_SWAP_SIZE_TOTAL: {
            mem_status.dwLength = sizeof(mem_status);
            if(GlobalMemoryStatusEx(&mem_status))
                return (int64_t) (mem_status.ullTotalPageFile
    ...
    switch(conf)
    {
        ...
        case CONF_CPU_L1_CACHE_SIZE: {
            ...
            if(sysctlbyname("hw.l1dcachesize", &l1_cache_size, &size, NULL, 0)
               == 0)
        ...
        case CONF_CPU_L2_CACHE_SIZE: {
#ifdef __linux__
            FILE *f =
                fopen("/sys/devices/system/cpu/cpu0/cache/index2/size", "r");
            if(!f)
                return -1;
            char size_str[32];
            if(fgets(size_str, sizeof(size_str), f))
            ...
            if(sysctlbyname("hw.l2cachesize", &l2_cache_size, &size, NULL, 0)
               == 0)
        ...
        case CONF_CPU_L3_CACHE_SIZE: {
            ...
            if(sysctlbyname("hw.l3cachesize", &l3_cache_size, &size, NULL, 0)
               == 0)
        ...
        case CONF_TIMEZONE_OFFSET: {
            ...
            return (int64_t) ((mktime(local_tm) - mktime(utc_tm))
                              / 60); // minute
        }
        case CONF_TTY_NAME_MAX: {
