...
template <typename T> struct _skiplist_node
{
    T obj;                           // Data object
    ...
    _skiplist_node (int level_, double score, const T &obj) :
        obj (obj), score (score), backward (nullptr)
    {
    }
};
...
template <typename T> class skiplist
{
  ...
  public:
    class iterator
    {
      ...
      public:
        ...
        {
            if (current)
                current = current->level_[0].forward;
            return *this;
        }
  ...
  public:
        ...
        for (int i = level_ - 1; i >= 0; i--) {
            ...
            while (x->level_[i].forward
                   && (x->level_[i].forward->score < score
                       || (x->level_[i].forward->score == score
                           && compare_ (x->level_[i].forward->obj, obj)))) {
    ...
    bool delete_node (double score, const T &obj)
    {
        ...
        if (x && x->score == score && !compare_ (x->obj, obj)
            && !compare_ (obj, x->obj)) {
    ...
    unsigned long get_rank (double score, const T &obj) const
    {
        ...
        for (int i = level_ - 1; i >= 0; i--) {
            ...
            if (x->level_[i].forward && x->level_[i].forward->score == score
                && !compare_ (x->level_[i].forward->obj, obj)
                && !compare_ (obj, x->level_[i].forward->obj)) {
                rank += x->level_[i].span;
    ...
    node_t *first_in_range (double min_score, double max_score) const
    {
        ...
        for (int i = level_ - 1; i >= 0; i--) {
            while (x->level_[i].forward
                   && x->level_[i].forward->score < min_score)
                x = x->level_[i].forward;
        }
    ...
    node_t *last_in_range (double min_score, double max_score) const
    {
        ...
        for (int i = level_ - 1; i >= 0; i--) {
            while (x->level_[i].forward
                   && x->level_[i].forward->score <= max_score)
                x = x->level_[i].forward;
        }
    ...
    unsigned long delete_range_by_rank (unsigned long start, unsigned long end)
    {
        ...
        for (int i = level_ - 1; i >= 0; i--) {
            while (x->level_[i].forward
                   && traversed + x->level_[i].span <= start) {
                traversed += x->level_[i].span;
                x = x->level_[i].forward;
            }
            update[i] = x;
  ...
  private:
    ...
    {
        size_t size =
          sizeof (node_t) + level_ * sizeof (typename node_t::_skiplist_level);
...
template <typename T> struct range_result
{
