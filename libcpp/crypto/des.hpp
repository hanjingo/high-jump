...
class des
{
  ...
  public:
        ...
        switch (algorithm) {
            case algo::des_ecb: {
                ...
                if (remain > 0 || _is_need_padding (pad_style)) {
                    ...
                    _padding_block (last_block, 8, src + idx, remain,
                                    pad_style);
                    DES_cblock block;
            ...
            case algo::des_ctr: {
                ...
                while (idx < padded_len) {
                    ...
                    if (idx + 8 <= src_len) {
                        ...
                        _padding_block (block, 8, src + idx, remain_bytes,
                                        pad_style);
                    } else {
                        ...
                        const std::string &iv = std::string ())
    {
        ...
        if (!encode (reinterpret_cast<unsigned char *> (
                     ...
                     key.size (), algorithm, pad_style, iv_ptr, iv.size ())) {
            dst.clear ();
            return false;
        }
                        ...
                        const std::size_t iv_len = 0)
    {
        ...
        while (true) {
            ...
            if (!is_last_block) {
                _stream_block_encode (key_schedules, algorithm, out, iv_block,
                                      ctr_block, inbuf, outbuf);
                continue;
            }
            ...
            if (read_len == block_size) {
                ...
                _padding_block (padding_block, block_size, nullptr, 0,
                                pad_style);
                _stream_block_encode (key_schedules, algorithm, out, iv_block,
                                      ctr_block, padding_block, outbuf);
            } else {
                // padding for the last block
                unsigned char padded_block[block_size];
                _padding_block (padded_block, block_size, inbuf, read_len,
                                pad_style);
                _stream_block_encode (key_schedules, algorithm, out, iv_block,
                                      ctr_block, padded_block, outbuf);
            }
                 ...
                 const std::size_t iv_len = 0)
    {
        ...
        return encode (dst_file, src_file, key, key_len, algorithm, pad_style,
                       iv, iv_len);
    }
                 ...
                 const std::string &iv = std::string ())
    {
        ...
        return encode_file (
          ...
          algorithm, pad_style, iv_ptr, iv.size ());
    }
                 ...
                 const std::size_t iv_len = 0)
    {
        ...
        return decode (dst_file, src_file, key, key_len, algorithm, pad_style,
                       iv, iv_len);
    }
  ...
  private:
    ...
    static std::size_t
    _multiple_key (std::vector<DES_key_schedule> &key_schedules,
                   const unsigned char *key,
                   const std::size_t key_len)
    {
    ...
    static void
    _stream_block_encode (std::vector<DES_key_schedule> &key_schedules,
