...
class rsa
{
  public:
    ...
    {
        x509,  // X.509 (-----BEGIN PUBLIC KEY-----) (-----BEGIN PRIVATE
               // KEY-----)
        pkcs1, // PKCS#1 (-----BEGIN RSA PUBLIC KEY-----) (-----BEGIN RSA
               // PRIVATE KEY-----)
    };
                        ...
                        const padding padding = padding::pkcs1)
    {
        ...
        int len = RSA_public_encrypt ((int) src_len, src, dst, rsa,
                        ...
                        const padding padding = padding::pkcs1)
    {
        ...
        if (!encode (
              ...
              dst_len, reinterpret_cast<const unsigned char *> (src.c_str ()),
              src.size (),
              reinterpret_cast<const unsigned char *> (pubkey_pem.c_str ()),
              pubkey_pem.size (), padding)) {
            dst.clear ();
            return false;
        }
                        ...
                        const padding padding = padding::pkcs1)
    {
        ...
        while (in) {
            ...
            write_len =
              RSA_public_encrypt (static_cast<int> (read_len), inbuf, outbuf,
                                  rsa, static_cast<int> (padding));
            if (write_len == -1) {
                RSA_free (rsa);
                return false;
            }
    ...
    {
        return encode_file (
          dst_file_path.c_str (), src_file_path.c_str (),
          reinterpret_cast<const unsigned char *> (pubkey_pem.c_str ()),
          pubkey_pem.size (), padding);
    }
    ...
    {
        RSA *rsa = _load_private_key (prikey_pem, prikey_pem_len, password,
                                      password_len);
        if (!rsa)
            return false;
        ...
        int len = RSA_private_decrypt (src_len, src, dst, rsa,
                                       static_cast<int> (padding));
        RSA_free (rsa);
        if (len == -1)
            return false;
                        ...
                        const std::string &password = "")
    {
        ...
        if (!decode (
              ...
              src.size (),
              reinterpret_cast<const unsigned char *> (prikey_pem.c_str ()),
              prikey_pem.size (), padding, password_ptr, password_len)) {
            dst.clear ();
            return false;
        }
                        ...
                        const std::size_t password_len = 0)
    {
        ...
        while (in) {
            ...
            int write_len =
              RSA_private_decrypt (static_cast<int> (read_len), inbuf, outbuf,
                                   rsa, static_cast<int> (padding));
            if (write_len == -1) {
                RSA_free (rsa);
                return false;
                             ...
                             const std::size_t password_len = 0)
    {
        ...
        std::ofstream out (reinterpret_cast<const char *> (dst_file_path),
                           std::ios::binary);
        return decode (out, in, prikey_pem, prikey_pem_len, padding, password,
                       password_len);
    }
                           ...
                           const std::size_t password_len = 0)
    {
        ...
        int len = RSA_private_encrypt (src_len, src, dst, rsa,
                                       static_cast<int> (padding));
        RSA_free (rsa);
        if (len == -1)
            return false;
                                  ...
                                  const padding padding = padding::pkcs1)
    {
        ...
        int len = RSA_public_decrypt (src_len, src, dst, rsa,
                                      static_cast<int> (padding));
        RSA_free (rsa);
        if (len == -1)
            return false;
                               ...
                               const std::size_t password_len = 0)
    {
        ...
        if (algorithm != algo::none
            && (!password
                || !_validate_password_for_cipher (algorithm, password_len)))
            return false;
                    ...
                    if (!pkey || !EVP_PKEY_set1_RSA (pkey, rsa)) {
                        ...
                        if (algorithm != algo::none)
                            pri_success = PEM_write_bio_PrivateKey (
                              pri_bio, pkey, _select_cipher (algorithm),
                              const_cast<unsigned char *> (password),
                              static_cast<int> (password_len), nullptr,
                              nullptr);
                        else
                            pri_success = PEM_write_bio_PrivateKey (
                              pri_bio, pkey, nullptr, nullptr, 0, nullptr,
                              nullptr);
                    }
                    break;
                case key_format::pkcs1:
                    ...
                    if (algorithm != algo::none)
                        pri_success = PEM_write_bio_RSAPrivateKey (
                          pri_bio, rsa, _select_cipher (algorithm),
                          const_cast<unsigned char *> (password),
                          static_cast<int> (password_len), nullptr, nullptr);
                    else
                        pri_success = PEM_write_bio_RSAPrivateKey (
                          pri_bio, rsa, nullptr, nullptr, 0, nullptr, nullptr);
                               ...
                               const std::string &password = "")
    {
        ...
        if (!make_key_pair (
              ...
              prikey_pem_len, bits, format, algorithm,
    ...
    static std::size_t encode_len_reserve (const unsigned char *pubkey_pem,
                                           const std::size_t pubkey_pem_len)
    {
        RSA *rsa = _load_public_key (pubkey_pem, pubkey_pem_len);
        if (!rsa)
    ...
    static std::size_t
    decode_len_reserve (const unsigned char *prikey_pem,
                        const std::size_t prikey_pem_len,
                        const unsigned char *password = nullptr,
                        const std::size_t password_len = 0)
    {
    ...
    {
        if (bits != 512 && bits != 1024 && bits != 2048 && bits != 3072
            && bits != 4096)
            return false;
  ...
  private:
                                  ...
                                  const std::size_t pubkey_pem_len)
    {
        ...
        BIO *bio =
          BIO_new_mem_buf (pubkey_pem, static_cast<int> (pubkey_pem_len));
        if (!bio)
            return nullptr;
                                   ...
                                   const std::string &password = std::string ())
    {
        ...
        return _load_private_key (
          reinterpret_cast<const unsigned char *> (prikey_pem.c_str ()),
          prikey_pem.size (), password_ptr, password_len);
    }
    ...
    static const EVP_CIPHER *_select_cipher (const algo algorithm)
    {
        switch (algorithm) {
            ...
            } // AES-128-CBC is the default for AES ECB
            case algo::aes_192_ecb: {
                return EVP_aes_192_cbc ();
            } // AES-192-CBC is the default for AES ECB
            case algo::aes_256_ecb: {
                return EVP_aes_256_cbc ();
            } // AES-256-CBC is the default for AES ECB
            ...
            } // 3DES-CBC is the default for 3DES ECB
            ...
            case algo::des_ecb: {
                return EVP_des_cbc ();
            } // DES-CBC is the default for DES ECB
            case algo::des_cbc: {
                return EVP_des_cbc ();
            }
            case algo::des_cfb: {
            ...
            case algo::bf_ecb: {
                return EVP_bf_cbc ();
            } // Blowfish-CBC is the default for Blowfish ECB
            case algo::bf_cbc: {
                return EVP_bf_cbc ();
            }
            case algo::bf_cfb: {
            ...
            } // CAST5-CBC is the default for CAST5 ECB
            case algo::cast5_cbc: {
                return EVP_cast5_cbc ();
            }
            case algo::cast5_cfb: {
            ...
            case algo::rc2_ecb: {
                return EVP_rc2_cbc ();
            } // RC2-CBC is the default for RC2 ECB
            case algo::rc2_cbc: {
                return EVP_rc2_cbc ();
            }
            case algo::rc2_cfb: {
    ...
    static bool _validate_password_for_cipher (const algo algorithm,
