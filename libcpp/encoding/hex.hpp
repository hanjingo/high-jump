...
namespace hex_detail
{
    ...
    get_type_tag() { return string_tag{}; }

    template<typename T>
    typename enable_if<is_same<T, std::vector<unsigned char>>::value, vector_tag>::type
    get_type_tag() { return vector_tag{}; }

    template<typename T>
    typename enable_if<std::is_integral<T>::value && 
                      !is_same<T, std::string>::value && 
                      !is_same<T, std::vector<unsigned char>>::value, integral_tag>::type
    get_type_tag() { return integral_tag{}; }

    template<typename T>
    typename enable_if<!std::is_integral<T>::value && 
                      !is_same<T, std::string>::value && 
                      !is_same<T, std::vector<unsigned char>>::value, container_tag>::type
    get_type_tag() { return container_tag{}; }
}
...
private:
    ...
    template<typename T>
    static T decode_impl_(const std::string& hex_str, hex_detail::container_tag)
    {
        T result;
        int high = 0, low = 0;
        for (std::size_t i = 0; i < hex_str.length(); i += 2) 
    ...
    template<typename T>
    static void encode_impl_(std::ostringstream& oss, const T& value, hex_detail::integral_tag)
    {
        oss << value;
    }

    template<typename T>
    static void encode_impl_(std::ostringstream& oss, const T& value, hex_detail::string_tag)
    {
    ...
    template<typename T>
    static void encode_impl_(std::ostringstream& oss, const T& value, hex_detail::vector_tag)
    {
    ...
    template<typename T>
    static void encode_impl_(std::ostringstream& oss, const T& value, hex_detail::container_tag)
    {
