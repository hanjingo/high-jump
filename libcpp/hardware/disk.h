...
extern "C"
{
    ...
    // -------------------------- disk API define -----------------------------
    disk_err_t disk_format_sz(uint64_t bytes, char *buffer, size_t buffer_size);
    disk_err_t disk_init(void);
    void disk_cleanup(void);
    uint32_t disk_count(void);
    disk_err_t disk_info(const char *device_name, disk_info_t *info);
    ...
    //     static inline disk_err_t
    //     disk_get_partition_by_mount(const char* mount_point,
    ...
    //         if (GetDiskFreeSpaceExA(mount_point, &free_bytes, &total_bytes,
    //                                 &total_free_bytes))
    //         {
    ...
    //         if (GetVolumeInformationA(mount_point, volume_name,
    //         sizeof(volume_name),
    //                                   &serial_number, &max_component_length,
    //                                   &filesystem_flags, filesystem_name,
    //                                   sizeof(filesystem_name)))
    //         {
    //             strncpy(info->volume_label, volume_name,
    //                     sizeof(info->volume_label) - 1);
    //             info->filesystem =
    //             filesystem_type_from_string(filesystem_name); info->read_only
    //             = (filesystem_flags & FILE_READ_ONLY_VOLUME) != 0;
    //         }
    ...
    //         sizeof(device_path)))
    //         {
    //             strncpy(info->device_name, device_path,
    ...
    //             if (sscanf(line, "%s %s %s", device, mountpoint, fstype) ==
    //             3)
    //             {
    //                 if (strcmp(mountpoint, mount_point) == 0)
    //                 {
    //                     strncpy(info->device_name, device,
    //                             sizeof(info->device_name) - 1);
    //                     info->filesystem =
    //                     filesystem_type_from_string(fstype); break;
    //                 }
    //             }
    //         }
    //         fclose(fp);
    ...
    //         strncpy(info->device_name, fs.f_mntfromname,
    //                 sizeof(info->device_name) - 1);
    ...
    //     static inline disk_err_t disk_is_ready(const char* device_name,
    //                                              bool* ready)
    //     {
    ...
    //     static inline disk_err_t disk_enumerate(disk_info_t* disks,
    //                                               uint32_t max_disks,
    ...
    //             if (drives & (1 << i))
    //             {
    //                 char drive_letter[4] = {(char) ('A' + i), ':', '\0',
    //                 '\0'}; UINT drive_type = GetDriveTypeA(drive_letter);

    //                 if (drive_type == DRIVE_FIXED || drive_type ==
    //                 DRIVE_REMOVABLE)
    //                 {
    //                     disk_err_t result =
    //                         disk_get_info(drive_letter,
    //                         &disks[*actual_count]);
    ...
    //     for (size_t i = 0; i < sizeof(disk_patterns) /
    //     sizeof(disk_patterns[0])
    //     &&
    ...
    //     static inline disk_err_t disk_test_read_speed(const char*
    //     device_name,
    //                                                     uint32_t
    //                                                     test_size_mb, double*
    //                                                     read_speed_mbps)
