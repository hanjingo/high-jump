...
typedef struct {
    char device_name[DISK_MAX_DEVICE_NAME];     /* Device name (e.g., "/dev/sda", "C:") */
...
static inline const char* disk_get_error_string(disk_error_t error) {
    switch (error) {
        ...
        case DISK_ERROR_INVALID_PARAMETER: return "Invalid parameter";
        case DISK_ERROR_ACCESS_DENIED: return "Access denied";
        case DISK_ERROR_NOT_FOUND: return "Not found";
        case DISK_ERROR_INSUFFICIENT_BUFFER: return "Insufficient buffer";
        case DISK_ERROR_IO_ERROR: return "I/O error";
        case DISK_ERROR_NOT_SUPPORTED: return "Not supported";
        case DISK_ERROR_INSUFFICIENT_MEMORY: return "Insufficient memory";
        case DISK_ERROR_SYSTEM_ERROR: return "System error";
        case DISK_ERROR_INVALID_FILESYSTEM: return "Invalid filesystem";
        case DISK_ERROR_DEVICE_BUSY: return "Device busy";
...
static inline const char* disk_type_to_string(disk_type_t type) {
    switch (type) {
        case DISK_TYPE_UNKNOWN: return "Unknown";
        case DISK_TYPE_HDD: return "Hard Disk Drive";
        case DISK_TYPE_SSD: return "Solid State Drive";
        case DISK_TYPE_OPTICAL: return "Optical Drive";
        case DISK_TYPE_REMOVABLE: return "Removable Drive";
        case DISK_TYPE_NETWORK: return "Network Drive";
        case DISK_TYPE_RAM: return "RAM Drive";
        default: return "Unknown";
    }
}

/* Convert filesystem type to string */
static inline const char* filesystem_type_to_string(filesystem_type_t fs_type) {
    switch (fs_type) {
        case FILESYSTEM_UNKNOWN: return "Unknown";
        case FILESYSTEM_NTFS: return "NTFS";
        case FILESYSTEM_FAT32: return "FAT32";
        case FILESYSTEM_EXFAT: return "exFAT";
        case FILESYSTEM_EXT2: return "ext2";
        case FILESYSTEM_EXT3: return "ext3";
        case FILESYSTEM_EXT4: return "ext4";
        case FILESYSTEM_XFS: return "XFS";
        case FILESYSTEM_BTRFS: return "Btrfs";
        case FILESYSTEM_ZFS: return "ZFS";
        case FILESYSTEM_HFS_PLUS: return "HFS+";
        case FILESYSTEM_APFS: return "APFS";
        case FILESYSTEM_UFS: return "UFS";
...
static inline filesystem_type_t filesystem_type_from_string(const char* fs_name) {
    if (!fs_name) return FILESYSTEM_UNKNOWN;
    
    if (strcmp(fs_name, "ntfs") == 0 || strcmp(fs_name, "NTFS") == 0) return FILESYSTEM_NTFS;
    if (strcmp(fs_name, "fat32") == 0 || strcmp(fs_name, "FAT32") == 0) return FILESYSTEM_FAT32;
    if (strcmp(fs_name, "exfat") == 0 || strcmp(fs_name, "exFAT") == 0) return FILESYSTEM_EXFAT;
    if (strcmp(fs_name, "ext2") == 0) return FILESYSTEM_EXT2;
    if (strcmp(fs_name, "ext3") == 0) return FILESYSTEM_EXT3;
    if (strcmp(fs_name, "ext4") == 0) return FILESYSTEM_EXT4;
    if (strcmp(fs_name, "xfs") == 0 || strcmp(fs_name, "XFS") == 0) return FILESYSTEM_XFS;
    if (strcmp(fs_name, "btrfs") == 0) return FILESYSTEM_BTRFS;
    if (strcmp(fs_name, "zfs") == 0 || strcmp(fs_name, "ZFS") == 0) return FILESYSTEM_ZFS;
    if (strcmp(fs_name, "hfs+") == 0 || strcmp(fs_name, "HFS+") == 0) return FILESYSTEM_HFS_PLUS;
    if (strcmp(fs_name, "apfs") == 0 || strcmp(fs_name, "APFS") == 0) return FILESYSTEM_APFS;
    if (strcmp(fs_name, "ufs") == 0 || strcmp(fs_name, "UFS") == 0) return FILESYSTEM_UFS;
    if (strcmp(fs_name, "iso9660") == 0 || strcmp(fs_name, "ISO9660") == 0) return FILESYSTEM_ISO9660;
    
    return FILESYSTEM_UNKNOWN;
}

/* Format size in bytes to human-readable string */
static inline disk_error_t disk_format_size(uint64_t bytes, char* buffer, size_t buffer_size) {
    ...
    int result;
    if (unit_index == 0) {
        result = snprintf(buffer, buffer_size, "%.0f %s", size, units[unit_index]);
    } else {
        result = snprintf(buffer, buffer_size, "%.2f %s", size, units[unit_index]);
    }
...
static inline disk_error_t disk_get_count(uint32_t* count) {
    ...
    *count = disk_count;
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (sscanf(line, "%lu %lu %lu %s", &major, &minor, &blocks, device_name) == 4) {
            /* Check if it's a whole disk (not a partition) */
            if (strstr(device_name, "sd") == device_name && strlen(device_name) == 3) {
                disk_count++;
...
static inline disk_error_t disk_get_info(const char* device_name, disk_info_t* info) {
    if (!device_name || !info) {
        return DISK_ERROR_INVALID_PARAMETER;
    }
    ...
    info->temperature = -1.0; /* Unknown temperature */
    ...
    HANDLE hDevice = CreateFileA(full_path, 0, FILE_SHARE_READ | FILE_SHARE_WRITE,
    ...
    switch (drive_type) {
        case DRIVE_FIXED:
            info->type = DISK_TYPE_HDD; /* Could be SSD, but can't determine easily */
            break;
    ...
    snprintf(sys_path, sizeof(sys_path), "/sys/block/%s/queue/rotational", 
             strrchr(device_name, '/') ? strrchr(device_name, '/') + 1 : device_name);
...
static inline disk_error_t disk_get_partition_by_mount(const char* mount_point, partition_info_t* info) {
    if (!mount_point || !info) {
        return DISK_ERROR_INVALID_PARAMETER;
    }
    ...
    if (GetVolumeInformationA(mount_point, volume_name, sizeof(volume_name),
                             &serial_number, &max_component_length, &filesystem_flags,
...
static inline disk_error_t disk_is_ready(const char* device_name, bool* ready) {
...
static inline disk_error_t disk_enumerate(disk_info_t* disks, uint32_t max_disks, uint32_t* actual_count) {
    ...
    *actual_count = 0;
    ...
    for (int i = 0; i < 26 && *actual_count < max_disks; i++) {
        if (drives & (1 << i)) {
            ...
            if (drive_type == DRIVE_FIXED || drive_type == DRIVE_REMOVABLE) {
                disk_error_t result = disk_get_info(drive_letter, &disks[*actual_count]);
    ...
    for (size_t i = 0; i < sizeof(disk_patterns) / sizeof(disk_patterns[0]) && *actual_count < max_disks; i++) {
        if (access(disk_patterns[i], F_OK) == 0) {
            disk_error_t result = disk_get_info(disk_patterns[i], &disks[*actual_count]);
            if (result == DISK_SUCCESS) {
                (*actual_count)++;
            }
        }
...
static inline disk_error_t disk_test_read_speed(const char* device_name, uint32_t test_size_mb, double* read_speed_mbps) {
    if (!device_name || !read_speed_mbps || test_size_mb == 0) {
        return DISK_ERROR_INVALID_PARAMETER;
    }
    
    /* This is a simplified implementation that would need platform-specific optimizations */
