...
// init context
bool gpu_context_create (gpu_context_t *ctx,
                         const gpu_device_info_t *device_info);

// build program
bool gpu_program_create_from_source (gpu_program_t *program,
                                     gpu_context_t *ctx,
                                     char *log,
                                     unsigned long *log_sz,
                                     const char *source);
// bool gpu_create_program_from_file(gpu_program_t* program, gpu_context_t*
...
bool gpu_malloc_read_only (gpu_buffer_t *buffer,
                           gpu_context_t *ctx,
                           size_t size);
bool gpu_malloc_write_only (gpu_buffer_t *buffer,
                            gpu_context_t *ctx,
                            size_t size);
...
// copy gpu memory
bool gpu_memcpy_to_device (gpu_buffer_t *dst,
                           const void *src,
                           size_t size,
                           gpu_context_t *ctx);
bool gpu_memcpy_from_device (void *dst,
                             const gpu_buffer_t *src,
                             size_t size,
                             gpu_context_t *ctx);
bool gpu_memcpy_device_to_device (gpu_buffer_t *dst,
                                  const gpu_buffer_t *src,
                                  size_t size,
                                  gpu_context_t *ctx);
bool gpu_memcpy_to_device_async (gpu_buffer_t *dst,
                                 const void *src,
                                 size_t size,
                                 gpu_context_t *ctx);
bool gpu_memcpy_from_device_async (void *dst,
                                   const gpu_buffer_t *src,
                                   size_t size,
                                   gpu_context_t *ctx);
bool gpu_memcpy_device_to_device_async (gpu_buffer_t *dst,
                                        const gpu_buffer_t *src,
                                        size_t size,
                                        gpu_context_t *ctx);

// set kernel arg
bool gpu_set_kernel_arg (gpu_program_t *program,
                         int arg_index,
                         size_t arg_size,
                         const void *arg_value);
bool gpu_set_kernel_arg_buffer (gpu_program_t *program,
                                int arg_index,
                                const gpu_buffer_t *buffer);

// exec kernel
bool gpu_execute_kernel_1d (gpu_program_t *program,
                            gpu_context_t *ctx,
                            size_t global_work_size,
                            size_t local_work_size);
bool gpu_execute_kernel_2d (gpu_program_t *program,
                            gpu_context_t *ctx,
                            size_t global_work_size[2],
                            size_t local_work_size[2]);
bool gpu_execute_kernel_3d (gpu_program_t *program,
                            gpu_context_t *ctx,
    ...
    *infos =
    ...
    for (cl_uint i = 0; i < num_platforms; ++i) {
        ...
        clGetDeviceIDs (platforms[i], CL_DEVICE_TYPE_GPU,
                        num_devices_in_platform, devices, NULL);
...
int gpu_device_get (gpu_device_info_t **infos, int *count)
{
    ...
    *infos =
    ...
    if (_gpu_get_devices_by_opencl (&opencl_infos, &opencl_count) == 0) {
        memcpy (*infos + current_index, opencl_infos,
    ...
    if (_gpu_get_devices_by_cuda (&cuda_infos, &cuda_count) == 0) {
        memcpy (*infos + current_index, cuda_infos,
                                     ...
                                     const char *source)
{
    ...
    if (err != CL_SUCCESS) {
        size_t sz = 0;
        clGetProgramBuildInfo (cl_prog, device, CL_PROGRAM_BUILD_LOG, 0, NULL,
                               &sz);
        if (sz > 0 && log && log_sz) {
            clGetProgramBuildInfo (cl_prog, device, CL_PROGRAM_BUILD_LOG, sz,
                                   log, NULL);
...
bool gpu_kernel_create (gpu_program_t *program, const char *kernel_name)
{
    ...
    CUfunction function;
    CUresult result =
      cuModuleGetFunction (&function, (CUmodule) program->program, kernel_name);
    if (result != CUDA_SUCCESS)
        return false;
                           ...
                           gpu_context_t *ctx)
{
    ...
    cl_int err = clEnqueueWriteBuffer (queue, mem_obj, CL_TRUE, 0, size, src, 0,
                                       NULL, NULL);
    if (err != CL_SUCCESS)
        return false;
                             ...
                             gpu_context_t *ctx)
{
    ...
    cl_int err = clEnqueueReadBuffer (queue, mem_obj, CL_TRUE, 0, size, dst, 0,
                                      NULL, NULL);
    if (err != CL_SUCCESS)
        return false;
                                  ...
                                  gpu_context_t *ctx)
{
    ...
    cudaError_t err = cudaMemcpyAsync (dst->ptr, src->ptr, size,
                                 ...
                                 gpu_context_t *ctx)
{
    ...
    cl_int err = clEnqueueWriteBuffer (queue, mem_obj, CL_FALSE, 0, size, src,
                                       0, NULL, NULL);
                                   ...
                                   gpu_context_t *ctx)
{
    ...
    cl_int err = clEnqueueReadBuffer (queue, mem_obj, CL_FALSE, 0, size, dst, 0,
                                      NULL, NULL);
    return (err == CL_SUCCESS);
...
bool gpu_set_kernel_arg_buffer (gpu_program_t *program,
                                ...
                                const gpu_buffer_t *buffer)
{
    if (!program || !program->kernel || !buffer || !buffer->ptr
        || arg_index < 0)
        return false;
...
#elif CUDA_ENABLE
    return gpu_set_kernel_arg (program, arg_index, sizeof (void *),
                               &buffer->ptr);
                            ...
                            size_t local_work_size)
{
    ...
    cl_int err = clEnqueueNDRangeKernel (
      queue, kernel, 1, NULL, &global_work_size, local_ptr, 0, NULL, NULL);
    if (err != CL_SUCCESS)
        return false;
    ...
    CUresult result = cuLaunchKernel (function, grid_size, 1,
                                      1, // grid dimensions
                                      block_size, 1,
                                      1,      // block dimensions
                            ...
                            size_t local_work_size[2])
{
    ...
    cl_int err = clEnqueueNDRangeKernel (
      queue, kernel, 2, NULL, global_work_size, local_ptr, 0, NULL, NULL);
    if (err != CL_SUCCESS)
        return false;
    ...
    CUresult result = cuLaunchKernel (function, grid_x, grid_y,
                                      1, // grid dimensions
                                      block_x, block_y,
                                      1,      // block dimensions
                                      0,      // shared memory size
                            ...
                            size_t local_work_size[3])
{
    ...
    cl_int err = clEnqueueNDRangeKernel (
      queue, kernel, 3, NULL, global_work_size, local_ptr, 0, NULL, NULL);
    if (err != CL_SUCCESS)
        return false;
