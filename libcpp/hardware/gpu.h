...
extern "C"
{
    ...
    typedef bool (*gpu_device_callback_t)(const gpu_device_info_t* info,
                                          void* user_data);
    typedef void (*gpu_error_callback_t)(const char* error_msg,
                                         void* user_data);
    ...
    static int _gpu_get_devices_by_opencl(gpu_device_info_t** infos, int* count)
    {
        ...
        *infos = (gpu_device_info_t*)calloc(total_devices,
                                            sizeof(gpu_device_info_t));
        if (!*infos)
        {
            free(platforms);
            return -1;
    ...
    *log_sz = 0;
                               ...
                               size_t local_work_size[3])
    {
        ...
        size_t* local_ptr = NULL;
        if (local_work_size && local_work_size[0] > 0 &&
            local_work_size[1] > 0 && local_work_size[2] > 0)
            local_ptr = local_work_size;
