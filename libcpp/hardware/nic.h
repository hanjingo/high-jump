...
// ------------------------------------ */ #define NIC_MAX_INTERFACES 32 #define
// NIC_MAX_NAME_LENGTH         256 #define NIC_MAX_DESCRIPTION_LENGTH  512
...
// typedef struct {
//     char name[NIC_MAX_NAME_LENGTH];                 /* Interface name (e.g.,
//     "eth0", "wlan0") */ char description[NIC_MAX_DESCRIPTION_LENGTH];   /*
//     Interface description */ char friendly_name[NIC_MAX_NAME_LENGTH]; /*
//     Friendly name (Windows) */ nic_mac_address_t mac_address; /* MAC address
//     */ nic_type_t type;                                /* Interface type */
//     nic_status_t status;                            /* Current status */
//     uint32_t index;                                 /* Interface index */
//     uint32_t mtu;                                   /* Maximum Transmission
//     Unit */ uint64_t speed;                                 /* Interface
//     speed in bps */ nic_duplex_t duplex;                            /* Duplex
//     mode */ bool dhcp_enabled;                              /* DHCP enabled
//     */ bool is_virtual;                                /* Is virtual
//     interface */ bool is_wireless;                               /* Is
//     wireless interface */ nic_ip_address_t ip_address;                    /*
//     Primary IPv4 address */ nic_ip_address_t subnet_mask; /* Subnet mask */
...
//     received */ uint64_t errors_sent;                           /* Send
//     errors */ uint64_t errors_received;                       /* Receive
//     errors */ uint64_t drops_sent;                            /* Sent packets
//     dropped */ uint64_t drops_received;                        /* Received
//     packets dropped */
// } nic_statistics_t;

// /* Wireless information (for wireless interfaces) */
// typedef struct {
//     char ssid[64];                                  /* SSID of connected
//     network */ char bssid[18];                                 /* BSSID (MAC
//     of access point) */ int signal_strength;                            /*
//     Signal strength in dBm */ int signal_quality; /* Signal quality
//     percentage */ uint32_t frequency;                             /*
//     Frequency in MHz */ char security_type[32];                         /*
//     Security type (WPA, WEP, etc.) */ bool is_connected; /* Connected to
//     network */
// } nic_wireless_info_t;
...
//         strncpy_s(nic->name, sizeof(nic->name), adapter->AdapterName,
//         _TRUNCATE); strncpy_s(nic->description, sizeof(nic->description),
//         adapter->Description, _TRUNCATE);

//         // Copy MAC address
//         if (adapter->AddressLength == NIC_MAC_ADDRESS_LENGTH) {
//             memcpy(nic->mac_address.bytes, adapter->Address,
//             NIC_MAC_ADDRESS_LENGTH);
//         }

//         nic->index = adapter->Index;
//         nic->type = (adapter->Type == MIB_IF_TYPE_ETHERNET) ?
//         NIC_TYPE_ETHERNET : NIC_TYPE_UNKNOWN; nic->dhcp_enabled =
...
//             nic->ip_address.ipv4.addr =
//             inet_addr(adapter->IpAddressList.IpAddress.String);
//         }
...
//             nic->subnet_mask.ipv4.addr =
//             inet_addr(adapter->IpAddressList.IpMask.String);
//         }
...
//             nic->gateway.ipv4.addr =
//             inet_addr(adapter->GatewayList.IpAddress.String);
//         }
...
//             nic->status = (ifa->ifa_flags & IFF_UP) ? NIC_STATUS_UP :
//             NIC_STATUS_DOWN;

//             // Get IP address
//             if (ifa->ifa_addr->sa_family == AF_INET) {
//                 struct sockaddr_in* sin = (struct sockaddr_in*)ifa->ifa_addr;
//                 inet_ntop(AF_INET, &sin->sin_addr, nic->ip_address.str,
//                 sizeof(nic->ip_address.str)); nic->ip_address.ipv4.addr =
//                 sin->sin_addr.s_addr;

//                 // Get netmask
//                 if (ifa->ifa_netmask) {
//                     struct sockaddr_in* netmask = (struct
//                     sockaddr_in*)ifa->ifa_netmask; inet_ntop(AF_INET,
//                     &netmask->sin_addr, nic->subnet_mask.str,
//                     sizeof(nic->subnet_mask.str)); nic->subnet_mask.ipv4.addr
//                     = netmask->sin_addr.s_addr;
//                 }
//             } else if (ifa->ifa_addr->sa_family == AF_INET6) {
//                 struct sockaddr_in6* sin6 = (struct
//                 sockaddr_in6*)ifa->ifa_addr; inet_ntop(AF_INET6,
//                 &sin6->sin6_addr, nic->ipv6_address.str,
...
//  * @return NIC_SUCCESS on success, error code on failure
//  */
// static inline nic_error_t nic_get_interface_info(const char* interface_name,
// nic_info_t* info) {
...
//     NIC_MAX_INTERFACES, &count); if (result != NIC_SUCCESS) {
//         return result;
//     }
...
//  * @return NIC_SUCCESS on success, error code on failure
//  */
// static inline nic_error_t nic_get_statistics(const char* interface_name,
...
//         if (sscanf(line, "%63[^:]: %lu %lu %lu %lu %*u %*u %*u %*u %lu %lu
//         %lu %lu",
...
//     char command[256];
//     snprintf(command, sizeof(command), "netsh interface set interface \"%s\"
//     enable", interface_name); int result = system(command); return (result ==
//     0) ? NIC_SUCCESS : NIC_ERROR_SYSTEM_ERROR;
// #else
//     return NIC_ERROR_NOT_SUPPORTED;
// #endif
// }
...
//     interface_name); int result = system(command); return (result == 0) ?
//     NIC_SUCCESS : NIC_ERROR_SYSTEM_ERROR;
// #elif defined(NIC_PLATFORM_WINDOWS)
//     char command[256];
//     snprintf(command, sizeof(command), "netsh interface set interface \"%s\"
//     disable", interface_name); int result = system(command); return (result
//     == 0) ? NIC_SUCCESS : NIC_ERROR_SYSTEM_ERROR;
// #else
//     return NIC_ERROR_NOT_SUPPORTED;
// #endif
...
//  * @return NIC_SUCCESS on success, error code on failure
//  */
// static inline nic_error_t nic_format_mac_address(const nic_mac_address_t*
// mac_addr,
//                                                  char* buffer, size_t
//                                                  buffer_size) {
...
//  * @return NIC_SUCCESS on success, error code on failure
//  */
// static inline nic_error_t nic_parse_mac_address(const char* mac_str,
// nic_mac_address_t* mac_addr) {
//     if (!mac_str || !mac_addr) {
//         return NIC_ERROR_INVALID_PARAMETER;
//     }
...
//  * @return NIC_SUCCESS on success, error code on failure
//  */
// static inline nic_error_t nic_is_wireless(const char* interface_name, bool*
// is_wireless) {
...
//     char command[256];
//     snprintf(command, sizeof(command), "iwconfig %s 2>/dev/null",
//     interface_name);
