...
// ------------------------------------ */ #define RAM_MAX_MODULES 8 #define
// RAM_MAX_MANUFACTURER_LENGTH     64 #define RAM_MAX_PART_NUMBER_LENGTH      32
...
//     ram_type_t type;                                    /* RAM type (DDR,
//     DDR2, etc.) */ uint64_t capacity;                                  /*
//     Capacity in bytes */ uint32_t speed; /* Speed in MHz */ uint32_t voltage;
//     /* Voltage in millivolts */ uint32_t slot_number; /* Physical slot number
...
// typedef struct {
//     uint64_t total_physical;                            /* Total physical RAM
//     in bytes */ uint64_t available_physical;                        /*
//     Available physical RAM in bytes */ uint64_t used_physical; /* Used
//     physical RAM in bytes */ uint64_t total_virtual; /* Total virtual memory
//     in bytes */ uint64_t available_virtual;                         /*
//     Available virtual memory in bytes */ uint64_t used_virtual; /* Used
//     virtual memory in bytes */ uint64_t total_swap; /* Total swap space in
//     bytes */ uint64_t available_swap;                            /* Available
//     swap space in bytes */ uint64_t used_swap; /* Used swap space in bytes */
//     uint32_t page_size;                                 /* Memory page size
//     in bytes */ uint32_t large_page_size;                           /* Large
//     page size in bytes */ double memory_load; /* Memory load percentage
//     (0-100) */ uint32_t module_count;                              /* Number
//     of RAM modules */
// } ram_system_info_t;

// /* Memory statistics */
// typedef struct {
//     uint64_t allocations_count;                         /* Number of
//     allocations */ uint64_t deallocations_count;                       /*
//     Number of deallocations */ uint64_t bytes_allocated; /* Total bytes
//     allocated */ uint64_t bytes_deallocated;                         /* Total
//     bytes deallocated */ uint64_t peak_usage; /* Peak memory usage */
//     uint64_t current_usage;                             /* Current memory
//     usage */ uint32_t page_faults;                               /* Page
//     fault count */ uint32_t page_file_usage;                           /*
//     Page file usage count */
// } ram_statistics_t;

// /* Memory region information */
// typedef struct {
//     void* base_address;                                 /* Base address of
//     region */ uint64_t size;                                      /* Size of
//     region in bytes */ ram_protection_t protection;                        /*
//     Memory protection flags */ bool is_committed; /* Is memory committed */
//     bool is_mapped;                                     /* Is memory mapped
//     */ bool is_shared;                                     /* Is shared
//     memory */
// } ram_region_info_t;
...
//         case RAM_ERROR_PROTECTION_VIOLATION: return "Memory protection
//         violation"; default: return "Unknown error";
//     }
// }
...
//     info->available_swap = memStatus.ullAvailPageFile -
//     memStatus.ullAvailPhys; info->used_swap = info->total_swap -
//     info->available_swap; info->memory_load = (double)memStatus.dwMemoryLoad;
...
//         info->memory_load = ((double)info->used_physical /
//         info->total_physical) * 100.0;
//     }
...
//         info->available_physical = (uint64_t)vm_stat.free_count *
//         page_size_vm; info->used_physical = info->total_physical -
...
//         strcpy_s(modules[0].manufacturer, sizeof(modules[0].manufacturer),
//         "Unknown"); strcpy_s(modules[0].part_number,
//         sizeof(modules[0].part_number), "System RAM"); modules[0].type =
//         RAM_TYPE_UNKNOWN; modules[0].capacity = sys_info.total_physical;
...
// #elif defined(RAM_PLATFORM_LINUX)
//     // On Linux, we can try to read from /proc/meminfo and
...
//     if (max_modules >= 1) {
//         memset(&modules[0], 0, sizeof(ram_module_info_t));
//         strncpy(modules[0].manufacturer, "Unknown",
//         sizeof(modules[0].manufacturer) - 1); strncpy(modules[0].part_number,
//         "System RAM", sizeof(modules[0].part_number) - 1); modules[0].type =
//         RAM_TYPE_UNKNOWN; modules[0].capacity = sys_info.total_physical;
//         modules[0].slot_number = 1;
//         *actual_count = 1;
//     }
...
//  * @return RAM_SUCCESS on success, error code on failure
//  */
// static inline ram_error_t ram_allocate_aligned(size_t size, size_t alignment,
// void** ptr) {
//     if (!ptr || size == 0 || alignment == 0 || (alignment & (alignment - 1))
//     != 0) {
//         return RAM_ERROR_INVALID_PARAMETER;
//     }
...
//     MEM_LARGE_PAGES, PAGE_READWRITE); return (*ptr != NULL) ? RAM_SUCCESS :
//     RAM_ERROR_ALLOCATION_FAILED;

// #elif defined(RAM_PLATFORM_LINUX)
//     *ptr = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_PRIVATE |
//     MAP_ANONYMOUS | MAP_HUGETLB, -1, 0); if (*ptr == MAP_FAILED) {
//         *ptr = NULL;
//         return RAM_ERROR_ALLOCATION_FAILED;
//     }
//     return RAM_SUCCESS;
...
//     *ptr = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_PRIVATE |
//     MAP_ANONYMOUS, -1, 0); if (*ptr == MAP_FAILED) {
//         *ptr = NULL;
//         return RAM_ERROR_ALLOCATION_FAILED;
//     }
//     return RAM_SUCCESS;
...
//  * @return RAM_SUCCESS on success, error code on failure
//  */
// static inline ram_error_t ram_protect_memory(void* ptr, size_t size,
...
//         case RAM_PROTECTION_READ_WRITE: win_protection = PAGE_READWRITE;
//         break; case RAM_PROTECTION_READ_EXECUTE: win_protection =
//         PAGE_EXECUTE_READ; break; case RAM_PROTECTION_READ_WRITE_EXECUTE:
//         win_protection = PAGE_EXECUTE_READWRITE; break; default: return
//         RAM_ERROR_INVALID_PARAMETER;
//     }
...
//  * @return RAM_SUCCESS on success, error code on failure
//  */
// static inline ram_error_t ram_format_size(uint64_t bytes, char* buffer,
// size_t buffer_size) {
...
//     } else {
//         snprintf(buffer, buffer_size, "%llu bytes", (unsigned long
//         long)bytes);
//     }
...
//         if (for_write) {
//             *is_valid = (mbi.Protect & (PAGE_READWRITE | PAGE_WRITECOPY |
//             PAGE_EXECUTE_READWRITE)) != 0;
//         } else {
//             *is_valid = (mbi.Protect & (PAGE_READONLY | PAGE_READWRITE |
//             PAGE_WRITECOPY |
