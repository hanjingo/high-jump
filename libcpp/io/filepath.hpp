...
public:
    ...
    static std::string relative(const std::string& path, const std::string& base = pwd())
    {
    ...
    static std::string path_name(const std::string& filepath)
    {
        ...
        return boost::filesystem::is_directory(path) ? filepath : path.parent_path().string();
    }
    ...
    static std::string replace_extension(const std::string& filepath, const std::string& ext)
    {
        return boost::filesystem::path(filepath).replace_extension(ext).string();
    }
    ...
    {
        if (recursive) {
            auto itr = boost::filesystem::recursive_directory_iterator(boost::filesystem::path(path));
        ...
        auto itr = boost::filesystem::directory_iterator(boost::filesystem::path(path));
    ...
    static std::list<std::string> find(const std::string& path, const std::string& file, bool recursive = false)
    {
    ...
    {
        return recursive ? boost::filesystem::create_directories(boost::filesystem::path(path))
    ...
    static unsigned long long remove(const std::string& filepath, bool recursive = true)
    {
        auto fpath = boost::filesystem::path(filepath);
        return recursive ? boost::filesystem::remove_all(fpath) : boost::filesystem::remove(fpath);
    }
