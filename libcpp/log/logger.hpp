                                   ...
                                   const QString& str)
{
    ...
    {
        case QtDebugMsg: {
            libcpp::logger::instance()->debug(str.toUtf8().constData());
            break;
        }
        case QtWarningMsg: {
            libcpp::logger::instance()->warn(str.toUtf8().constData());
            break;
        }
        case QtCriticalMsg: {
            libcpp::logger::instance()->critical(str.toUtf8().constData());
            break;
        }
        case QtInfoMsg: {
            libcpp::logger::instance()->info(str.toUtf8().constData());
            break;
        }
        default:
            break;
...
class logger
{
  ...
  public:
           ...
           const uint64_t thread_num = LOG_THREAD_NUM)
    {
        ...
        {
            base_ = std::make_shared<spdlog::logger>(name,
                                                     sinks.begin(),
                                                     sinks.end());
        }
    ...
    inline void remove_sink(const sink_ptr_t sink)
    {
        std::lock_guard<std::mutex> lock(log_mu);
        base_->sinks().erase(
            std::remove(base_->sinks().begin(), base_->sinks().end(), sink),
            base_->sinks().end());
    }
    ...
    inline log_lvl get_level() { return static_cast<log_lvl>(base_->level()); }
...
#define LOG_TRACE(...) libcpp::logger::instance()->trace(__VA_ARGS__)

#define LOG_DEBUG(...) libcpp::logger::instance()->debug(__VA_ARGS__)

#define LOG_INFO(...) libcpp::logger::instance()->info(__VA_ARGS__)

#define LOG_WARN(...) libcpp::logger::instance()->warn(__VA_ARGS__)

#define LOG_ERROR(...) libcpp::logger::instance()->error(__VA_ARGS__)

#define LOG_CRITICAL(...) libcpp::logger::instance()->critical(__VA_ARGS__)

#define LOG_FLUSH() libcpp::logger::instance()->flush();
