...
template <typename T> class matrix
{
  public:
    ...
    matrix (const std::vector<std::vector<T> > &buf) :
        row_n_{int (buf.size ())}, col_n_{row_n_ > 0 ? int (buf[0].size ()) : 0}
    {
        buf_ = create_ ();
    ...
    inline matrix_row_iterator<matrix<T>, T> vfind (const int row,
                                                    const int col)
    {
  ...
  private:
    ...
    template <typename Container> void copy_from_ (const Container &rhs)
    {
