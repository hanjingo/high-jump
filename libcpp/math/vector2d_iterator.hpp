...
template <typename Vec, typename T> struct vector2d_iterator
{
    ...
    vector2d_iterator () : vec_{0}, row_{0}, col_{0}, n_row_{0}, n_col_{0} {}
    ...
    inline T &operator* () { return at (row_, col_); }
    ...
    inline Vec *vector () { return vec_; }

    virtual T &at (const int row, const int col) { return (*vec_)[row][col]; }
