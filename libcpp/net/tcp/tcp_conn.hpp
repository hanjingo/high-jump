...
class tcp_conn
{
  public:
    ...
    using encode_handler_t = std::function<
        std::size_t(unsigned char*, const std::size_t, msg_ptr_t)>;
    using decode_handler_t = std::function<
        std::size_t(msg_ptr_t, const unsigned char*, const std::size_t)>;
  ...
  public:
    ...
    {
        return sock_ != nullptr && sock_->is_connected();
    }
  ...
  private:
    ...
    void _async_recv(const err_t& err, std::size_t sz)
    {
        ...
        do
        {
            ...
            auto data =
                boost::asio::buffer_cast<const unsigned char*>(r_buf_.data());
            sz = r_buf_.size();
