...
class tcp_socket
{
  ...
  public:
    tcp_socket () = delete;
    explicit tcp_socket (io_t &io) : io_{io} {}
    explicit tcp_socket (io_t &io, sock_t *sock) : io_{io}, sock_{sock} {}
    ...
    {
        return sock_ != nullptr && is_connected_.load ();
    }
    ...
    void
    async_connect (const char *ip, const uint16_t port, conn_handler_t &&fn)
    {
        endpoint_t ep{address_t::from_string (ip), port};
    ...
    size_t recv_until (streambuf_t &buf, size_t least)
    {
        ...
        err_t err;
        auto sz = boost::asio::read (
          *sock_, buf, boost::asio::transfer_at_least (least), err);
        if (err.failed ())
            assert (false);
