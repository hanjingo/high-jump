...
static const char* kernel_version(char* buffer, size_t buffer_size) 
{
    ...
    {
        snprintf(buffer, buffer_size, "%lu.%lu.%lu",
                info.dwMajorVersion,
                info.dwMinorVersion,
                info.dwBuildNumber);
        return buffer;
...
static uint64_t kernel_uptime(void)
{
#if defined(_WIN32) || defined(_WIN64)
...
static const char* kernel_uptime_str(char* buffer, size_t buffer_size, const char* fmt)
{
    if (!buffer || buffer_size == 0)
        return NULL;
...
static bool kernel_info(kernel_info_t* info)
{
    if (!info)
        return false;
