...
static constexpr boost::process::detail::std_in_<void> std_in =
  boost::process::std_in;
static constexpr boost::process::detail::std_out_<1> std_out =
  boost::process::std_out;
static constexpr boost::process::detail::std_out_<2> std_err =
...
template <typename... Args> inline int system (Args &&...args)
{
    return boost::process::system (std::forward<Args> (args)...);
}

template <typename... Args> inline child_t child (Args &&...args)
{
    return boost::process::child (std::forward<Args> (args)...);
}

template <typename... Args> inline void spawn (Args &&...args)
{
    boost::process::spawn (std::forward<Args> (args)...);
}

template <typename... Args> inline void daemon (Args &&...args)
{
    // Step1. ignore signal
    ...
    boost::process::spawn (std::forward<Args> (args)...,
                           boost::process::start_dir (
                             boost::filesystem::current_path ()), // pwd/root
                           boost::process::std_in.null (),
                           boost::process::std_out.null (),
                           boost::process::std_out.null (), gp);
    gp.detach ();
}
...
inline void list (
  ...
  list_match_cb match = [] (const std::vector<std::string>) -> bool {
      return true;
  })
{
    ...
    try {
        boost::process::child proc (cmd_list_pid,
