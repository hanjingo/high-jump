...
{
  public:
    crash_handler () : _dump_cb{default_dump_callback} {}
    crash_handler (const char *abs_path) : _dump_cb{default_dump_callback}
    {
    ...
    bool prevent_set_unhandled_exception_filter ()
    {
        ...
        void *pOrgEntry = (void *) (::GetProcAddress (
          hKernel32, "SetUnhandledExceptionFilter"));
        if (pOrgEntry == NULL)
            return false;
        ...
        BOOL bRet = ::WriteProcessMemory (::GetCurrentProcess (), pOrgEntry,
                                          newJump, 5, &bytesWritten);
