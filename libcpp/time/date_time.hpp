...
{
// ASIA
constexpr timezone_info BEIJING = {"Asia/Shanghai", "CST", 8, 0,
                                   "China Standard Time"};
constexpr timezone_info HONG_KONG = {"Asia/Hong_Kong", "HKT", 8, 0,
                                     "Hong Kong Time"};
constexpr timezone_info TOKYO = {"Asia/Tokyo", "JST", 9, 0,
                                 "Japan Standard Time"};
constexpr timezone_info SEOUL = {"Asia/Seoul", "KST", 9, 0,
                                 "Korea Standard Time"};
constexpr timezone_info MUMBAI = {"Asia/Kolkata", "IST", 5, 30,
                                  "India Standard Time"};
constexpr timezone_info BANGKOK = {"Asia/Bangkok", "ICT", 7, 0,
                                   "Indochina Time"};
constexpr timezone_info SINGAPORE = {"Asia/Singapore", "SGT", 8, 0,
                                     "Singapore Standard Time"};
constexpr timezone_info DUBAI = {"Asia/Dubai", "GST", 4, 0,
                                 "Gulf Standard Time"};

// EUROPE
constexpr timezone_info LONDON = {"Europe/London", "GMT", 0, 0,
                                  "Greenwich Mean Time"};
constexpr timezone_info BERLIN = {"Europe/Berlin", "CET", 1, 0,
                                  "Central European Time"};
constexpr timezone_info PARIS = {"Europe/Paris", "CET", 1, 0,
                                 "Central European Time"};
constexpr timezone_info ROME = {"Europe/Rome", "CET", 1, 0,
                                "Central European Time"};
constexpr timezone_info MOSCOW = {"Europe/Moscow", "MSK", 3, 0,
                                  "Moscow Standard Time"};
constexpr timezone_info STOCKHOLM = {"Europe/Stockholm", "CET", 1, 0,
                                     "Central European Time"};

// AMERICAS
constexpr timezone_info NEW_YORK = {"America/New_York", "EST", -5, 0,
                                    "Eastern Standard Time"};
constexpr timezone_info CHICAGO = {"America/Chicago", "CST", -6, 0,
                                   "Central Standard Time"};
constexpr timezone_info DENVER = {"America/Denver", "MST", -7, 0,
                                  "Mountain Standard Time"};
constexpr timezone_info LOS_ANGELES = {"America/Los_Angeles", "PST", -8, 0,
                                       "Pacific Standard Time"};
constexpr timezone_info TORONTO = {"America/Toronto", "EST", -5, 0,
                                   "Eastern Standard Time"};
constexpr timezone_info SAO_PAULO = {"America/Sao_Paulo", "BRT", -3, 0,
                                     "Brazil Time"};
constexpr timezone_info MEXICO_CITY = {"America/Mexico_City", "CST", -6, 0,
                                       "Central Standard Time"};

// AUSTRALIA & OCEANIA
constexpr timezone_info SYDNEY = {"Australia/Sydney", "AEST", 10, 0,
                                  "Australian Eastern Standard Time"};
constexpr timezone_info MELBOURNE = {"Australia/Melbourne", "AEST", 10, 0,
                                     "Australian Eastern Standard Time"};
constexpr timezone_info AUCKLAND = {"Pacific/Auckland", "NZST", 12, 0,
                                    "New Zealand Standard Time"};
constexpr timezone_info PERTH = {"Australia/Perth", "AWST", 8, 0,
                                 "Australian Western Standard Time"};

// AFRICA
constexpr timezone_info CAIRO = {"Africa/Cairo", "EET", 2, 0,
                                 "Eastern European Time"};
constexpr timezone_info JOHANNESBURG = {"Africa/Johannesburg", "SAST", 2, 0,
                                        "South Africa Standard Time"};
constexpr timezone_info LAGOS = {"Africa/Lagos", "WAT", 1, 0,
                                 "West Africa Time"};

// other
constexpr timezone_info UTC = {"UTC", "UTC", 0, 0,
...
class date_time
{
  ...
  public:
    date_time () : tm_{NullTime} {};
    date_time (const date_time &dt) : tm_{dt.tm_} {};
  ...
  public:
    ...
    {
        static libcpp::date_time::is_working_day_fn inst =
          [] (const libcpp::date_time &dt) -> bool {
            auto day =
              static_cast<week_day> (int64_t (dt.tm_.date ().day_of_week ()));
    ...
    static date_time now (const timezone_info &tz = timezone::LOCAL)
    {
        if (tz.name == timezone::UTC.name) {
            ...
            auto utc_time =
              boost::posix_time::microsec_clock::universal_time ();
            auto offset = boost::posix_time::time_duration (
              tz.hours_offset, tz.minutes_offset, 0);
    ...
    static date_time today (const timezone_info &tz = timezone::LOCAL)
    {
        if (tz.name == timezone::UTC.name) {
            ...
            auto offset = boost::posix_time::time_duration (
              tz.hours_offset, tz.minutes_offset, 0);
            return date_time (utc_day, offset.hours (), offset.minutes (), 0,
                              0);
        }
    }
  ...
  public:
    inline bool is_null () { return NullTime == tm_; }

    inline bool is_bigger (const date_time &dt) { return tm_ > dt.tm_; }

    inline bool is_smaller (const date_time &dt) { return tm_ < dt.tm_; }

    inline bool is_equal (const date_time &dt) { return tm_ == dt.tm_; }
    ...
    inline std::tm date () { return boost::posix_time::to_tm (tm_); }

    inline std::time_t time () { return boost::posix_time::to_time_t (tm_); }

    inline int64_t seconds () { return tm_.time_of_day ().seconds (); }
    ...
    inline int64_t minute () { return tm_.time_of_day ().minutes (); }
    ...
    inline int64_t hour () { return tm_.time_of_day ().hours (); }
    ...
    inline int64_t day () { return tm_.date ().day (); }
    ...
    inline int64_t day_of_month () { return tm_.date ().day (); }

    inline int64_t day_of_year () { return tm_.date ().day_of_year (); }
    ...
    inline int64_t year () { return tm_.date ().year (); }

    inline date_time start_of_day () { return date_time (tm_.date ()); }

    inline date_time end_of_day ()
    {
        return date_time (tm_.date ().year (), tm_.date ().month (),
                          tm_.date ().day (), 23, 59, 59);
    }
    ...
    inline date_time start_of_week ()
    {
        return day_of_week () == week_day::sunday
                 ? date_time (tm_.date ()
                              + boost::gregorian::date_duration (-6))
                     .start_of_day ()
                 : date_time (tm_.date ()
    ...
    {
        return date_time (tm_.date ().year (),
                          static_cast<unsigned short> (month ()), 1);
    }
    ...
    {
        return static_cast<int64_t> (month ())
                   <= static_cast<int64_t> (moon::june)
                 ? date_time (tm_.date ().year (), 1, 1)
    ...
    inline date_time end_of_half_year ()
    {
        return static_cast<int64_t> (month ())
                   <= static_cast<int64_t> (moon::june)
                 ? date_time (tm_.date ().year (), 6, 30, 23, 59, 59)
    ...
    inline date_time next_month (const unsigned long n = 1)
    {
        ...
        return date_time (boost::gregorian::date (target_date.year (),
                                                  target_date.month (), 1));
    }
