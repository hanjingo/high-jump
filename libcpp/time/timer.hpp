...
static std::unordered_map<int64_t, std::shared_ptr<libcpp::timer> >
  tm_map_global{};
...
class timer
{
  public:
    ...
    timer (unsigned long long us, F &&f, int64_t id = -1) :
        id_{id}, tm_{io_global, boost::posix_time::microseconds (us)}, fn_{}
    {
        auto fn = std::move (f);
        fn_ = std::bind (
    ...
    template <typename F> inline void reset (unsigned long long us, F &&f)
    {
        auto fn = std::move (f);
        fn_ = std::bind (
          [=] (const boost::system::error_code &err) {
    ...
    inline void wait () { tm_.wait (); }

    template <typename F>
    static std::shared_ptr<timer> create_global (unsigned long long us, F &&f)
    {
        auto tm =
          std::make_shared<libcpp::timer> (us, std::move (f), tm_id_tag++);
