...
{
  public:
    template <typename T> static bool get (const T src, const unsigned int pos)
    {
        return (src >> (pos - 1)) & 1;
    }

    template <typename T> static T &put (T &src, const unsigned int pos)
    {
        return src |= (1 << (pos - 1));
    }
    ...
    template <typename T> static T &reset (T &src, const bool bit)
    {
        src = bit ? (~T (0)) : (0);
        return src;
    }

    template <typename T> static T &flip (T &src)
    {
        src = ~(src);
        return src;
    }

    template <typename T> static void to_string (const T &src, char *buf)
    {
    ...
    template <typename T> static std::string to_string (const T &src)
    {
    ...
    template <typename T> static int count_leading_zeros (const T &src)
    {
