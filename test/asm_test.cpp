...
TEST_F (AsmTest, asm_macro_compilation)
{
    ...
    // Note: On x64 MSVC, inline assembly is not supported, so we expect this to
    // work only on supported platforms
...
// // Test multithreaded compilation (macros should be thread-safe for
// compilation) TEST_F(AsmTest, multithreaded_compilation_safety)
// {
...
//             try {
//                 // Test that macros can be used safely in multithreaded
//                 compilation context
//                 // This tests the macro definitions themselves, not assembly
//                 execution

//                 volatile int test_var = i;

//                 // Use macros in a way that tests compilation but doesn't
//                 execute assembly
// #if !defined(_MSC_VER) || (!defined(_M_X64) && !defined(_M_ARM64))
...
//     std::cout << "Multithreaded compilation safety test completed" <<
//     std::endl;
// }
...
TEST_F (AsmTest, header_include_guard)
{
    ...
    // This is tested by including the header multiple times (implicitly through
    // test framework)
...
#if !defined(_MSC_VER) || (!defined(_M_X64) && !defined(_M_ARM64))
        // On supported platforms, the macros exist but we don't use them for
        // actual assembly
#endif
