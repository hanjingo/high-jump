...
class AStarTest : public ::testing::Test
{
  protected:
    ...
    grid_t create_grid_with_obstacles (int width,
                                       int height,
                                       const std::vector<location_t> &obstacles)
    {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                if (std::find (obstacles.begin (), obstacles.end (), current)
                    != obstacles.end ()) {
                ...
                for (const auto &neighbor : potential_neighbors) {
                    if (neighbor.x >= 0 && neighbor.x < width && neighbor.y >= 0
                        && neighbor.y < height
                        && std::find (obstacles.begin (), obstacles.end (),
                                      neighbor)
                             == obstacles.end ()) {
    ...
    grid_t create_maze_grid ()
    {
        ...
        std::vector<location_t> walls = {
          {1, 1}, {1, 2}, {1, 3}, {1, 5}, {2, 3}, {2, 5}, {3, 1}, {3, 3},
          {3, 5}, {4, 1}, {4, 3}, {5, 1}, {5, 2}, {5, 3}, {5, 4}, {5, 5}};
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                if (std::find (obstacles.begin (), obstacles.end (), current)
                    != obstacles.end ()) {
                    std::cout << "# "; // Obstacle
                } else if (std::find (path.begin (), path.end (), current)
                           != path.end ()) {
    ...
    bool is_valid_path (const std::vector<location_t> &path,
                        const grid_t &test_grid)
    {
        if (path.empty ())
            return false;
        if (path.size () == 1)
            return true; // Single point path is valid
        ...
        for (size_t i = 0; i < path.size () - 1; ++i) {
            ...
            try {
                ...
                if (std::find (neighbors.begin (), neighbors.end (), next)
                    == neighbors.end ()) {
                    std::cout << "Invalid step from (" << current.x << ","
                              << current.y << ") to (" << next.x << ","
                              << next.y << ")" << std::endl;
                    return false;
                }
                ...
                std::cout << "Exception checking neighbors for (" << current.x
                          << "," << current.y << "): " << e.what ()
                          << std::endl;
                return false;
            }
    ...
    bool location_exists_in_grid (const location_t &loc,
                                  const grid_t &test_grid)
    {
        try {
    ...
    double calculate_path_cost (const std::vector<location_t> &path,
                                const grid_t &test_grid)
    {
        if (path.size () < 2)
            return 0.0;
...
TEST_F (AStarTest, search_without_smoothing)
{
    ...
    while (!frontier.empty ()) {
        ...
        for (const location_t &next : neighbors) {
            cost_t new_cost =
              cost_so_far[current] + test_grid.cost (current, next);
            if (cost_so_far.find (next) == cost_so_far.end ()
                || new_cost < cost_so_far[next]) {
                cost_so_far[next] = new_cost;
                cost_t priority =
                  new_cost + detail::manhattan_heuristic (next, goal);
    ...
    for (size_t i = 0; i < path.size () - 1; ++i) {
        ...
        EXPECT_TRUE (std::find (neighbors.begin (), neighbors.end (), next)
                     != neighbors.end ())
          << "Invalid step from (" << current.x << "," << current.y << ") to ("
          << next.x << "," << next.y << ")";
    }
...
TEST_F (AStarTest, search_with_obstacles_detailed)
{
    ...
    for (int y = 0; y < 3; y++) {
        for (int x = 0; x < 3; x++) {
            ...
            try {
                ...
                std::cout << "(" << x << "," << y
                          << ") - error getting neighbors" << std::endl;
            }
        }
    }
    ...
    if (path.size () > 0) {
        ...
        for (const auto &loc : path) {
            EXPECT_EQ (std::find (obstacles.begin (), obstacles.end (), loc),
                       obstacles.end ());
        }
...
TEST_F (AStarTest, path_optimality_check)
{
    ...
    if (path.size () > 0) {
        // For a 3x3 grid_t from (0,0) to (2,2), optimal path length should be 5
        // steps (0,0) -> (1,0) -> (2,0) -> (2,1) -> (2,2) or similar
...
TEST_F (AStarTest, large_grid_optimal_path)
{
    ...
    if (path.size () > 0) {
        ...
        std::cout << "Expected minimum (Manhattan): " << (2 * size - 2)
                  << std::endl;
    }
}
...
TEST_F (AStarTest, spiral_path_navigation)
{
    ...
    std::vector<location_t> spiral_walls = {{1, 1}, {2, 1}, {3, 1}, {3, 2},
                                            {3, 3}, {2, 3}, {1, 3}, {1, 2}};
...
TEST_F (AStarTest, performance_timing)
{
    ...
    std::cout << "Performance test (" << size << "x" << size
