...
TEST (base64, encode)
{
    ...
    std::string str_dst;
    ASSERT_EQ (libcpp::base64::encode (
                 str_dst, std::string ("https://github.com/hanjingo/libcpp")),
               true);
...
TEST (base64, decode)
{
    ...
    unsigned char buf_dst[1024];
    std::size_t buf_dst_len = 1024;
    unsigned char buf[] = {'a', 'G', 'V', 's', 'b', 'G', '8', 'g',
                           'b', 'G', 'l', 'j', 'c', 'H', 'A', '='};
...
{
    // base64 file -> file
    ASSERT_EQ (libcpp::base64::encode_file (
...
{
    ASSERT_EQ (libcpp::base64::decode_file (
