...
TEST (crc16, standard_test_vectors)
{
    ...
    EXPECT_EQ (result1, 0x4B37)
      << "Standard test vector '123456789' failed, actual: 0x" << std::hex
      << result1;
#elif CRC16_ALGO == CRC16_XMODEM
    EXPECT_EQ (result1, 0x31C3)
      << "Standard test vector '123456789' failed, actual: 0x" << std::hex
      << result1;
#endif
...
#if CRC16_ALGO == CRC16_MODBUS
    EXPECT_EQ (result2, 0xFFFF)
      << "Empty string test failed, actual: 0x" << std::hex << result2;
#elif CRC16_ALGO == CRC16_XMODEM
    EXPECT_EQ (result2, 0x0000)
      << "Empty string test failed, actual: 0x" << std::hex << result2;
#endif
...
#if CRC16_ALGO == CRC16_MODBUS
    EXPECT_NE (result3, 0)
      << "Single character 'a' should produce non-zero CRC";
#elif CRC16_ALGO == CRC16_XMODEM
    EXPECT_NE (result3, 0)
      << "Single character 'a' should produce non-zero CRC";
#endif
...
#if CRC16_ALGO == CRC16_MODBUS
    EXPECT_EQ (crc16 (nullptr, 0, 0), 0xFFFF)
      << "CRC of null data with zero length should be initial value (0xFFFF)";
    EXPECT_EQ (crc16 (nullptr, 10, 123), 123);
    EXPECT_EQ (crc16 (data, 0, 0), 0xFFFF)
      << "CRC of zero length data should be initial value (0xFFFF)";
#elif CRC16_ALGO == CRC16_XMODEM
    EXPECT_EQ (crc16 (nullptr, 0, 0), 0x0000)
      << "CRC of null data with zero length should be initial value (0x0000)";
    EXPECT_EQ (crc16 (nullptr, 10, 123), 123);
    EXPECT_EQ (crc16 (data, 0, 0), 0x0000)
      << "CRC of zero length data should be initial value (0x0000)";
#endif
}
