...
{
    ASSERT_STREQ (
      libcpp::filepath::dir_name ("/usr/local/src/007.txt").c_str (), "src");
}
...
{
    ASSERT_STREQ (
      libcpp::filepath::extension ("/usr/local/src/007.txt").c_str (), ".txt");
}
...
TEST (file_path, list)
{
    ASSERT_EQ (libcpp::filepath::list (libcpp::filepath::pwd ()).size () > 0,
               true);
}
...
TEST (file_path, make_dir)
{
    ...
    ASSERT_EQ ((libcpp::filepath::is_exist (path)
                  ? true
...
TEST (file_path, copy_file)
{
    std::string from =
      libcpp::filepath::join (libcpp::filepath::pwd (), "007.txt");
    std::string to =
      libcpp::filepath::join (libcpp::filepath::pwd (), "008.txt");
    ASSERT_EQ (
      (libcpp::filepath::is_exist (from) && (!libcpp::filepath::is_exist (to))
