...
{
    ASSERT_STREQ(libcpp::filepath::parent("/usr/local/src").c_str(),
                 "/usr/local");
}
...
{
#if !defined(_WIN32)
    ASSERT_STREQ(libcpp::filepath::absolute("/usr/local/src").c_str(),
                 "/usr/local/src");
#endif
...
{
#if defined(_WIN32)
    ASSERT_STREQ(libcpp::filepath::relative("/usr/local/src/007.txt").c_str(),
...
{
#if defined(_WIN32)
    ASSERT_STREQ(libcpp::filepath::join("/usr/local/src", "007.txt").c_str(),
                 "/usr/local/src\\007.txt");
    std::vector<std::string> args{ "/usr/local/src", "007.txt" };
    ASSERT_STREQ(libcpp::filepath::join(args).c_str(),
                 "/usr/local/src\\007.txt");
#else
    ASSERT_STREQ(libcpp::filepath::join("/usr/local/src", "007.txt").c_str(),
                 "/usr/local/src/007.txt");
...
{
    ASSERT_STREQ(libcpp::filepath::file_name("/usr/local/src/007.txt").c_str(),
                 "007.txt");
}

TEST(file_path, dir_name)
{
    ASSERT_STREQ(libcpp::filepath::dir_name("/usr/local/src/007.txt").c_str(),
                 "src");
}

TEST(file_path, path_name)
{
    ASSERT_STREQ(libcpp::filepath::path_name("/usr/local/src/007.txt").c_str(),
                 "/usr/local/src");
}

TEST(file_path, extension)
{
    ASSERT_STREQ(libcpp::filepath::extension("/usr/local/src/007.txt").c_str(),
                 ".txt");
}
...
{
    ASSERT_STREQ(
        libcpp::filepath::replace_extension("/usr/local/src/007.txt", ".exe")
            .c_str(),
...
{
    ASSERT_EQ(libcpp::filepath::last_mod_time(libcpp::filepath::pwd()) > 0,
              true);
}
...
TEST(file_path, size)
{
    ...
    std::string f = libcpp::filepath::join(libcpp::filepath::pwd(),
                                           "file_path_size_test.txt");
...
{
    int n = 0;
    libcpp::filepath::walk(libcpp::filepath::pwd(),
...
{
    std::string f = libcpp::filepath::join(libcpp::filepath::pwd(),
                                           "file_path_find_test.txt");
    if (!libcpp::filepath::is_exist(f))
    {
        ASSERT_EQ(libcpp::filepath::make_file(f), true);
    }

    ASSERT_EQ(libcpp::filepath::find(libcpp::filepath::pwd(),
                                     "file_path_find_test.txt")
                      .size() > 0,
              true);
}
...
TEST(file_path, make_dir)
{
    ...
    ASSERT_EQ(
        (libcpp::filepath::is_exist(path) ? true
                                          : libcpp::filepath::make_dir(path)),
        true);
}
...
TEST(file_path, copy_dir)
{
    ...
    ASSERT_EQ(
        (libcpp::filepath::is_exist(from) && (!libcpp::filepath::is_exist(to))
             ? libcpp::filepath::copy_dir(from, to)
             : true),
        true);
}
...
{
    std::string from =
        libcpp::filepath::join(libcpp::filepath::pwd(), "007.txt");
    std::string to = libcpp::filepath::join(libcpp::filepath::pwd(), "008.txt");
    ASSERT_EQ(
        (libcpp::filepath::is_exist(from) && (!libcpp::filepath::is_exist(to))
             ? libcpp::filepath::copy_file(from, to)
             : true),
        true);
}
...
TEST(file_path, rename)
{
    ...
    ASSERT_STREQ(
        (libcpp::filepath::is_exist(from) && (!libcpp::filepath::is_exist(to))
             ? libcpp::filepath::rename(from, to).c_str()
             : to.c_str()),
        to.c_str());
}
