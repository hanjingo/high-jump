...
TEST(ini, parse)
{
    ...
    ASSERT_EQ(
        ini.get_child("section1").get<std::string>("b") == std::string("abc"),
        true);
    ASSERT_EQ(ini.get_child("section1").get<float>("c") == float(123.456),
              true);
}
...
TEST(ini, read_file)
{
    ...
    ASSERT_EQ(cfg2.get_child("person").get<std::string>("name") ==
                  std::string("hanjingo"),
              true);
    ASSERT_EQ(cfg2.get_child("person").get<int>("age") == 30, true);
    ASSERT_EQ(cfg2.get_child("person").get<float>("income") == float(10000.123),
              true);
}
...
TEST(ini, write_file)
{
    ...
    libcpp::ini cfg2;
    cfg2.read_file("cfg.ini");
    ASSERT_EQ(cfg2.get_child("person").get<std::string>("email") ==
