...
TEST (pdf, show_text_at)
{
    ...
    page.begin_text ();
    EXPECT_NO_THROW (
      page.show_text_at ("Hello, PDF!", libcpp::pdf::point (100.0f, 200.0f)));
    page.end_text ();
}
...
TEST (pdf, draw_rectangle)
{
    libcpp::pdf::document doc;
    auto &page = doc.add_page ();
    EXPECT_NO_THROW (page.draw_rectangle (
      libcpp::pdf::rectangle (100.0f, 200.0f, 50.0f, 30.0f), true));
}
...
TEST (pdf, draw_circle)
{
    ...
    auto &page = doc.add_page ();
    EXPECT_NO_THROW (
      page.draw_circle (libcpp::pdf::point (150.0f, 250.0f), 25.0f, true));
}
...
TEST (pdf, draw_line)
{
    libcpp::pdf::document doc;
    auto &page = doc.add_page ();
    page.draw_line (libcpp::pdf::point (100.0f, 200.0f),
                    libcpp::pdf::point (200.0f, 300.0f), true);
}
...
TEST (pdf, draw_text)
{
    ...
    page.begin_text ();
    EXPECT_NO_THROW (
      page.draw_text ("Hello, PDF!", libcpp::pdf::point (100.0f, 200.0f)));
    page.end_text ();
}
...
TEST (pdf, draw_image)
{
    ...
    // {
    //     EXPECT_NO_THROW(page.draw_image("test_image.png",
    //     libcpp::pdf::rectangle(100.0f, 200.0f, 50.0f, 50.0f)));
    //     EXPECT_NO_THROW(page.draw_image("test_image.png",
    //     libcpp::pdf::point(100.0f, 200.0f), 50.0f, 50.0f));
    // }
}
...
TEST (pdf, insert_image)
{
    ...
    // libcpp::pdf::document doc;
    // auto& page = doc.add_page();
    // page.insert_image("image.png", libcpp::pdf::rectangle(100.0f,
    // 200.0f, 50.0f, 50.0f)); page.insert_image("image.png",
    // libcpp::pdf::point(100.0f, 200.0f), 50.0f, 50.0f);
}
...
TEST (pdf, get_permission)
{
    ...
    doc.set_password (
      ...
      "owner_password"); // Set password before setting permissions
    doc.set_permission (libcpp::pdf::document::permission::print);
    ASSERT_TRUE (doc.get_permission ()
                 == libcpp::pdf::document::permission::print);
}
...
TEST (pdf, clear_all_permissions)
{
    ...
    ASSERT_TRUE (doc.get_permission ()
                 == libcpp::pdf::document::permission::none);
}
