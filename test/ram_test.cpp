...
// protected:
//     void SetUp() override {
//         std::cout << "\n=== Random Access Memory (RAM) Tests Setup ===" <<
//         std::endl;

//         // Initialize RAM subsystem
//         ram_error_t result = ram_init();
//         ASSERT_EQ(result, RAM_SUCCESS) << "Failed to initialize RAM
//         subsystem: "
...
//         std::cout << "=== Random Access Memory (RAM) Tests Teardown ===" <<
//         std::endl; ram_cleanup();
//     }
// };
...
//     result = ram_init();
//     EXPECT_EQ(result, RAM_SUCCESS) << "Multiple RAM initializations should be
//     safe";
...
//     std::cout << "RAM initialization and cleanup tests completed" <<
//     std::endl;
// }
...
//     EXPECT_STREQ(ram_get_error_string(RAM_ERROR_INVALID_PARAMETER), "Invalid
//     parameter"); EXPECT_STREQ(ram_get_error_string(RAM_ERROR_ACCESS_DENIED),
//     "Access denied"); EXPECT_STREQ(ram_get_error_string(RAM_ERROR_NOT_FOUND),
//     "Memory region not found");
//     EXPECT_STREQ(ram_get_error_string(RAM_ERROR_INSUFFICIENT_BUFFER),
//     "Insufficient buffer");
//     EXPECT_STREQ(ram_get_error_string(RAM_ERROR_SYSTEM_ERROR), "System
//     error"); EXPECT_STREQ(ram_get_error_string(RAM_ERROR_NOT_SUPPORTED),
//     "Operation not supported");
//     EXPECT_STREQ(ram_get_error_string(RAM_ERROR_INSUFFICIENT_MEMORY),
//     "Insufficient memory");
//     EXPECT_STREQ(ram_get_error_string(RAM_ERROR_ALLOCATION_FAILED), "Memory
//     allocation failed");
//     EXPECT_STREQ(ram_get_error_string(RAM_ERROR_INVALID_ADDRESS), "Invalid
//     memory address");
//     EXPECT_STREQ(ram_get_error_string(RAM_ERROR_PROTECTION_VIOLATION),
//     "Memory protection violation");

//     // Test unknown error code
//     EXPECT_STREQ(ram_get_error_string(static_cast<ram_error_t>(-999)),
//     "Unknown error");
...
//     std::cout << "Testing system memory information retrieval..." <<
//     std::endl;

//     // Test invalid parameter
//     ram_error_t result = ram_get_system_info(nullptr);
//     EXPECT_EQ(result, RAM_ERROR_INVALID_PARAMETER) << "Should return error
//     for null parameter";
...
//         std::cout << "  Memory Load: " << info.memory_load << "%" <<
//         std::endl; std::cout << "  Page Size: " << info.page_size << " bytes"
//         << std::endl; std::cout << "  Large Page Size: " <<
//         info.large_page_size << " bytes" << std::endl; std::cout << "  Module
//         Count: " << info.module_count << std::endl;

//         // Validate the information
//         EXPECT_GT(info.total_physical, 0) << "Total physical memory should be
//         greater than 0"; EXPECT_LE(info.used_physical, info.total_physical)
//         << "Used memory should not exceed total";
//         EXPECT_LE(info.available_physical, info.total_physical) << "Available
//         memory should not exceed total"; EXPECT_GE(info.memory_load, 0.0) <<
//         "Memory load should be non-negative"; EXPECT_LE(info.memory_load,
//         100.0) << "Memory load should not exceed 100%";
...
//             std::endl; std::cout << "    Part Number: " << module.part_number
//             << std::endl; std::cout << "    Serial Number: " <<
//             module.serial_number << std::endl; std::cout << "    Type: " <<
//             ram_type_to_string(module.type) << std::endl;

//             char capacity_str[64];
//             ram_format_size(module.capacity, capacity_str,
//             sizeof(capacity_str)); std::cout << "    Capacity: " <<
//             capacity_str << std::endl;

//             std::cout << "    Speed: " << module.speed << " MHz" <<
//             std::endl; std::cout << "    Voltage: " << module.voltage << "
//             mV" << std::endl; std::cout << "    Slot Number: " <<
//             module.slot_number << std::endl; std::cout << "    ECC: " <<
//             (module.is_ecc ? "Yes" : "No") << std::endl; std::cout << "
//             Registered: " << (module.is_registered ? "Yes" : "No") <<
//             std::endl;

//             // Validate module data
//             EXPECT_GT(module.capacity, 0) << "Module capacity should be
//             greater than 0"; EXPECT_GT(module.slot_number, 0) << "Slot number
//             should be greater than 0";
//         }

//         EXPECT_GE(actual_count, 1) << "Should have at least one RAM module";
//         EXPECT_LE(actual_count, RAM_MAX_MODULES) << "Module count should not
//         exceed maximum";
//     } else {
//         std::cout << "RAM module enumeration not supported on this platform"
//         << std::endl;
//     }
...
//         result = ram_allocate_aligned(test_size, alignments[i],
//         &aligned_ptr); EXPECT_EQ(result, RAM_SUCCESS) << "Aligned allocation
//         should succeed for alignment " << alignments[i];

//         if (result == RAM_SUCCESS) {
//             EXPECT_NE(aligned_ptr, nullptr) << "Allocated pointer should not
//             be null";

//             // Check alignment
//             uintptr_t addr = (uintptr_t)aligned_ptr;
//             EXPECT_EQ(addr % alignments[i], 0) << "Memory should be aligned
//             to " << alignments[i] << " bytes";
...
//                 EXPECT_EQ(byte_ptr[j], 0xAA) << "Memory content should be
//                 preserved";
//             }

//             // Free the memory
//             result = ram_free_aligned(aligned_ptr);
//             EXPECT_EQ(result, RAM_SUCCESS) << "Aligned memory deallocation
//             should succeed";
//         }
//     }
...
//     // Test large page allocation (may fail if not supported or insufficient
//     privileges) const size_t large_page_size = 2 * 1024 * 1024; // 2 MB
...
//             EXPECT_EQ(byte_ptr[i], 0x55) << "Large page memory content should
//             be preserved";
//         }

//         // Free the large pages
//         result = ram_free_large_pages(ptr, large_page_size);
//         EXPECT_EQ(result, RAM_SUCCESS) << "Large page deallocation should
//         succeed";

//     } else {
//         std::cout << "Large page allocation not supported or insufficient
//         privileges: "
...
//     // Test invalid parameters
//     result = ram_protect_memory(nullptr, test_size,
//     RAM_PROTECTION_READ_WRITE); EXPECT_EQ(result,
//     RAM_ERROR_INVALID_PARAMETER) << "Should return error for null pointer";
...
//         RAM_PROTECTION_NONE
//     };
//     const size_t num_protections = sizeof(protections) /
//     sizeof(protections[0]);

//     for (size_t i = 0; i < num_protections; i++) {
//         result = ram_protect_memory(ptr, test_size, protections[i]);

//         if (result == RAM_SUCCESS) {
//             std::cout << "Successfully set protection mode " <<
//             protections[i] << std::endl;
...
//                 uint8_t* byte_ptr = (uint8_t*)ptr;
//                 byte_ptr[0] = 0x42;
//                 EXPECT_EQ(byte_ptr[0], 0x42) << "Write should succeed with
//                 write protection";
//             }
//         } else {
//             std::cout << "Protection mode " << protections[i] << " not
//             supported: "
//                       << ram_get_error_string(result) << std::endl;
//         }
//     }
...
//             EXPECT_EQ(byte_ptr[0], 0x33) << "Unlocked memory should still be
//             accessible";
//         }
//     } else {
//         std::cout << "Memory locking not supported or insufficient
//         privileges: "
//                   << ram_get_error_string(result) << std::endl;
//     }
...
//         }
//     } else {
//         std::cout << "Memory prefetching not supported: " <<
//         ram_get_error_string(result) << std::endl;
//     }
...
//     EXPECT_STREQ(ram_format_size(1023, buffer, sizeof(buffer)), "1023
//     bytes"); EXPECT_STREQ(ram_format_size(1024, buffer, sizeof(buffer)), "1
//     KB"); EXPECT_STREQ(ram_format_size(2 * 1024, buffer, sizeof(buffer)), "2
//     KB"); EXPECT_STREQ(ram_format_size(1024 * 1024, buffer, sizeof(buffer)),
//     "1 MB"); EXPECT_STREQ(ram_format_size(2 * 1024 * 1024, buffer,
//     sizeof(buffer)), "2 MB"); EXPECT_STREQ(ram_format_size(1024 * 1024 *
//     1024, buffer, sizeof(buffer)), "1 GB"); EXPECT_STREQ(ram_format_size(2 *
//     1024 * 1024 * 1024, buffer, sizeof(buffer)), "2 GB");
