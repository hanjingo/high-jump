...
-----END PUBLIC KEY-----)";
    ...
    std::memcpy (plain_data.data (), original_msg.data (),
                 original_msg.size ());
    unsigned char encoded[4096];
    std::size_t encoded_len = 4096;
    ASSERT_EQ (libcpp::rsa::encode (
                 encoded, encoded_len, plain_data.data (), plain_data.size (),
                 reinterpret_cast<unsigned char *> (pubkey.data ()),
                 pubkey.size (), libcpp::rsa::padding::no_padding),
               true);
    unsigned char decoded[4096];
    std::size_t decoded_len = 4096;
    ASSERT_EQ (
      libcpp::rsa::decode (decoded, decoded_len, encoded, encoded_len,
                           reinterpret_cast<unsigned char *> (prikey.data ()),
                           prikey.size (), libcpp::rsa::padding::no_padding),
      true);
    ASSERT_EQ (decoded_len, key_size);
    ASSERT_EQ (std::memcmp (decoded, plain_data.data (), key_size), 0);
    ASSERT_EQ (
      std::memcmp (decoded, original_msg.data (), original_msg.size ()), 0);
    ...
    ASSERT_EQ (
      libcpp::rsa::decode (
        decoded_stream, decoded_stream_len,
        reinterpret_cast<const unsigned char *> (encoded_stream_str.c_str ()),
        encoded_stream_str.size (),
        reinterpret_cast<unsigned char *> (prikey.data ()), prikey.size (),
...
-----END PUBLIC KEY-----)";
    ...
    ASSERT_EQ (
      libcpp::rsa::encode (
        encoded, encoded_len,
        reinterpret_cast<const unsigned char *> (plain.c_str ()), plain.size (),
        reinterpret_cast<unsigned char *> (pubkey.data ()), pubkey.size ()),
      true);
    ...
    ASSERT_EQ (libcpp::rsa::decode (
                 out, in, reinterpret_cast<unsigned char *> (prikey.data ()),
                 prikey.size ()),
               true);
...
-----END RSA PRIVATE KEY-----)";
    ...
    ASSERT_EQ (libcpp::rsa::decode_file (
                 ...
                 std::string ("./rsa_file_test_encode.log"), prikey),
               true);
}
...
TEST (rsa, make_key_pair)
{
    ...
    std::string plain = "hello world";
    ASSERT_EQ (libcpp::rsa::make_key_pair (
                 pubkey, prikey, 2048, libcpp::rsa::key_format::x509,
                 libcpp::rsa::algo::aes_256_cbc, passwd),
               true);
    ...
    std::size_t decoded_len = 4096;
    ASSERT_EQ (libcpp::rsa::decode (
                 decoded, decoded_len, encoded, encoded_len,
...
-----END PUBLIC KEY-----)";
    ...
    ASSERT_EQ (libcpp::rsa::is_pubkey_valid (
                 ...
                 pubkey1.size (), libcpp::rsa::padding::pkcs1),
               true);
    ...
    ASSERT_EQ (libcpp::rsa::is_pubkey_valid (
                 ...
                 pubkey2.size (), libcpp::rsa::padding::pkcs1),
               false);
}
...
-----END RSA PRIVATE KEY-----)";
    ...
    ASSERT_EQ (libcpp::rsa::is_prikey_valid (
                 ...
                 prikey1.size (), libcpp::rsa::padding::pkcs1),
               true);
    ...
    ASSERT_EQ (libcpp::rsa::is_prikey_valid (
                 ...
                 prikey2.size (), libcpp::rsa::padding::pkcs1),
               false);
}