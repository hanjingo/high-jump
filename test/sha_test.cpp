...
{
    std::string data;
    ASSERT_EQ (libcpp::sha::encode (data, std::string ("123456"),
                                    libcpp::sha::algorithm::sha256),
               true);
    ASSERT_EQ (data.empty (), false);
    ASSERT_STREQ (data.c_str (),
                  "\x8D\x96\x9E\xEFn\xCA\xD3\xC2\x9A:b\x92\x80\xE6\x86\xCF\f?]"
                  "Z\x86\xAF\xF3\xCA\x12\x2\f\x92:\xDCl\x92");

    unsigned char src[] = {'a', 'b', 'c'};
    unsigned char dst[1024];
    std::size_t dst_len = 1024;
    ASSERT_EQ (libcpp::sha::encode (dst, dst_len, src, 3,
                                    libcpp::sha::algorithm::sha256),
               true);
