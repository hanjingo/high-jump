...
class SkipListTest : public ::testing::Test
{
  protected:
    ...
    template <typename T> void print_skiplist_debug (const skiplist<T> &sl)
    {
    ...
    template <typename T> bool verify_order (const skiplist<T> &sl)
    {
...
TEST_F (SkipListTest, get_range_by_score)
{
    ...
    std::vector<std::string> expected_limited = {"cherry", "date",
                                                 "elderberry"};
...
TEST_F (SkipListTest, performance_test)
{
    ...
    std::cout << "Inserted " << num_elements << " elements in "
    ...
    std::cout << "1000 rank-based searches took " << duration.count () << " μs"
              << std::endl;
}
