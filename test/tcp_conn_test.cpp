...
class my_codec
{
  public:
    ...
    static std::size_t decode (libcpp::tcp_conn::msg_ptr_t msg,
...
TEST (tcp_conn, async_connect)
{
    ...
    int connect_times = 0;
    libcpp::tcp_conn conn1{io};
    ASSERT_EQ (conn1.async_connect (
                 "127.0.0.1", 10011,
                 [&connect_times] (libcpp::tcp_conn::conn_ptr_t conn,
                                   const libcpp::tcp_conn::err_t &err) {
...
TEST (tcp_conn, send)
{
    std::thread t1 ([] () {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            sock.set_decode_handler (
              std::bind (my_codec::decode, std::placeholders::_1,
    ...
    conn1.set_send_handler (
      [] (libcpp::tcp_conn::conn_ptr_t conn, libcpp::tcp_conn::msg_ptr_t msg) {
          (void) conn;
          ASSERT_EQ (msg != nullptr, true);
      });
    conn1.set_encode_handler (
      std::bind (my_codec::encode, std::placeholders::_1, std::placeholders::_2,
                 std::placeholders::_3));
    ...
    conn2.set_encode_handler (
      [] (unsigned char *data, const std::size_t len,
          libcpp::tcp_conn::msg_ptr_t msg) -> std::size_t {
...
TEST (tcp_conn, recv)
{
    std::thread t1 ([] () {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            libcpp::tcp_conn sock{io, base};
            sock.set_encode_handler (
              std::bind (my_codec::encode, std::placeholders::_1,
                         std::placeholders::_2, std::placeholders::_3));
    ...
    libcpp::tcp_conn::io_t io;
    libcpp::tcp_conn conn1{io};
    conn1.set_decode_handler (
      std::bind (my_codec::decode, std::placeholders::_1, std::placeholders::_2,
                 std::placeholders::_3));
...
TEST (tcp_conn, async_send)
{
    ...
    libcpp::tcp_conn::io_t io;
    libcpp::tcp_conn conn1{io};
    conn1.set_send_handler ([&nsend] (libcpp::tcp_conn::conn_ptr_t conn,
                                      libcpp::tcp_conn::msg_ptr_t msg) {
        (void) conn;
        nsend++;
        ASSERT_EQ (msg != nullptr, true);
    ...
    libcpp::tcp_conn conn2{io};
    conn2.set_send_handler ([&nsend] (libcpp::tcp_conn::conn_ptr_t conn,
                                      libcpp::tcp_conn::msg_ptr_t msg) {
        (void) conn;
        nsend++;
        ASSERT_EQ (msg != nullptr, true);
...
TEST (tcp_conn, async_recv)
{
    ...
    conn1.set_decode_handler (
      std::bind (my_codec::decode, std::placeholders::_1, std::placeholders::_2,
                 std::placeholders::_3));
    conn1.set_recv_handler ([&nrecv] (libcpp::tcp_conn::conn_ptr_t conn,
                                      libcpp::tcp_conn::msg_ptr_t arg) {
        (void) conn;
        nrecv++;
    ...
    conn2.set_decode_handler (
      std::bind (my_codec::decode, std::placeholders::_1, std::placeholders::_2,
                 std::placeholders::_3));
    conn2.set_recv_handler ([&nrecv] (libcpp::tcp_conn::conn_ptr_t conn,
                                      libcpp::tcp_conn::msg_ptr_t arg) {
        (void) conn;
        nrecv++;
        auto msg = static_cast<message *> (arg);
