...
TEST (tcp_listener, async_accept)
{
    ...
    std::thread t1 ([] () {
        libcpp::tcp_socket::io_t io;
        libcpp::tcp_listener li{io, [] (const libcpp::tcp_listener::err_t &err,
                                        libcpp::tcp_socket *sock) {
                                    ASSERT_EQ (err.failed (), false);
    ...
    std::thread t2 ([] () {
        ...
        libcpp::tcp_listener li{io};
        for (int i = 0; i < 2; i++) {
            li.async_accept (10092, [] (const libcpp::tcp_listener::err_t &err,
                                        libcpp::tcp_socket *sock) {
                ASSERT_EQ (err.failed (), false);
                ASSERT_EQ (sock->is_connected (), true);
                ASSERT_EQ (sock->check_connected (), true);
    ...
    sock2.async_connect ("127.0.0.1", 10092,
    ...
    sock3.async_connect ("127.0.0.1", 10092,
