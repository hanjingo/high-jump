...
//     sock.signal_catch({SIGILL}, [](const libcpp::tcp_socket::err_t& err,
//     libcpp::tcp_socket::signal_t sig)->bool{
...
//     sock.signal_catch({SIGINT, SIGILL}, [](const libcpp::tcp_socket::err_t&
//     err, libcpp::tcp_socket::signal_t sig)->bool{
...
TEST (tcp_socket, set_option)
{
    libcpp::tcp_socket::io_t io;
    libcpp::tcp_socket sock{io};
    ASSERT_EQ (sock.set_option (libcpp::tcp_socket::opt_no_delay (true)),
               false);
    ASSERT_EQ (sock.set_option (libcpp::tcp_socket::opt_send_buf_sz (1024)),
               false);
    ASSERT_EQ (sock.set_option (libcpp::tcp_socket::opt_recv_buf_sz (1024)),
               false);
    ASSERT_EQ (sock.set_option (libcpp::tcp_socket::opt_reuse_addr (true)),
               false);
    ASSERT_EQ (sock.set_option (libcpp::tcp_socket::opt_keep_alive (false)),
               false);
    // ASSERT_EQ(sock.set_option(libcpp::tcp_socket::opt_broadcast(false)),
    // false);

    libcpp::tcp_socket::sock_t *base1 = new libcpp::tcp_socket::sock_t (io);
    libcpp::tcp_socket sock1{io, base1};
    ASSERT_EQ (sock1.set_option (libcpp::tcp_socket::opt_no_delay (true)),
               false);
    ASSERT_EQ (sock1.set_option (libcpp::tcp_socket::opt_send_buf_sz (1024)),
               false);
    ASSERT_EQ (sock1.set_option (libcpp::tcp_socket::opt_recv_buf_sz (1024)),
               false);
    ASSERT_EQ (sock1.set_option (libcpp::tcp_socket::opt_reuse_addr (true)),
               false);
    ASSERT_EQ (sock1.set_option (libcpp::tcp_socket::opt_keep_alive (false)),
               false);
    // ASSERT_EQ(sock1.set_option(libcpp::tcp_socket::opt_broadcast(false)),
    // false);
    ...
    ASSERT_EQ (sock2.set_option (libcpp::tcp_socket::opt_no_delay (true)),
               true);
    ASSERT_EQ (sock2.set_option (libcpp::tcp_socket::opt_send_buf_sz (1024)),
               true);
    ASSERT_EQ (sock2.set_option (libcpp::tcp_socket::opt_recv_buf_sz (1024)),
               true);
    ASSERT_EQ (sock2.set_option (libcpp::tcp_socket::opt_reuse_addr (true)),
               true);
    ASSERT_EQ (sock2.set_option (libcpp::tcp_socket::opt_keep_alive (false)),
               true);
    // ASSERT_EQ(sock2.set_option(libcpp::tcp_socket::opt_broadcast(false)),
    // true);
}
...
TEST (tcp_socket, connect)
{
    ...
    ASSERT_EQ (
      sock1.connect ("127.0.0.1", 10091, std::chrono::milliseconds (30)), true);

    std::this_thread::sleep_for (std::chrono::milliseconds (10));
    libcpp::tcp_socket sock2{io};
    ASSERT_EQ (
      sock2.connect ("127.0.0.1", 10091, std::chrono::milliseconds (20), 5),
      true);
...
TEST (tcp_socket, async_connect)
{
    ...
    libcpp::tcp_listener li{io};
    li.async_accept (
      10091, [&li, &accept_times] (const libcpp::tcp_listener::err_t &err,
          ...
          li.async_accept (
            10091, [&accept_times] (const libcpp::tcp_listener::err_t &err,
                                    libcpp::tcp_socket *sock) {
                ASSERT_EQ (err.failed (), false);
                ASSERT_EQ (sock != nullptr, true);
                accept_times++;
...
TEST (tcp_socket, send)
{
    ...
    ASSERT_EQ (sock1.send (reinterpret_cast<const unsigned char *> (
...
TEST (tcp_socket, async_send)
{
    std::thread t ([] () {
        libcpp::tcp_socket::io_t io;
        libcpp::tcp_listener li{io};
        li.async_accept (10091, [] (const libcpp::tcp_listener::err_t &err,
                                    libcpp::tcp_socket *sock) {
            ASSERT_EQ (err.failed (), false);
            ASSERT_EQ (sock->is_connected (), true);
            ...
            sock->async_recv (
              buf, 1024,
              [sock, &buf] (const libcpp::tcp_socket::err_t &err,
                            std::size_t sz) {
                  ASSERT_EQ (err.failed (), false);
                  ASSERT_EQ (sz == 5, true);
    ...
    libcpp::tcp_socket sock{io};
    sock.async_connect (
      "127.0.0.1", 10091, [&sock] (const libcpp::tcp_socket::err_t &err) {
          ASSERT_EQ (err.failed (), false);
          ASSERT_EQ (sock.is_connected (), true);
          ASSERT_EQ (sock.check_connected (), true);
          ...
          sock.async_send (
            reinterpret_cast<const unsigned char *> (
              std::string ("hello").c_str ()),
            5, [] (const libcpp::tcp_socket::err_t &err, std::size_t sz) {
                ASSERT_EQ (err.failed (), false);
                ASSERT_EQ (sz == 5, true);
            });
      });
...
TEST (tcp_socket, async_recv)
{
    std::thread t ([] () {
        ...
        li.async_accept (10091, [&buf, &nrecved, &sock_ptr] (
            ...
            sock->async_recv (
              buf, 1024,
              [&buf, &nrecved] (const libcpp::tcp_socket::err_t &err,
                                std::size_t sz) {
                  ASSERT_EQ (err.failed (), false);
                  ASSERT_EQ (sz >= 5, true);
                  nrecved += sz;
