...
TEST(tcp_socket, connect)
{
    ...
    ASSERT_EQ(sock1.connect("127.0.0.1", 10091, std::chrono::milliseconds(30)),
              true);
...
TEST(tcp_socket, async_connect)
{
    ...
    li.async_accept(
        10091,
        [&li, &accept_times](const libcpp::tcp_listener::err_t& err,
            ...
            li.async_accept(
                10091,
                [&accept_times](const libcpp::tcp_listener::err_t& err,
                                libcpp::tcp_socket* sock) {
                    ASSERT_EQ(err.failed(), false);
                    ASSERT_EQ(sock != nullptr, true);
                    accept_times++;
               ...
               libcpp::tcp_socket* sock) {
                ...
                sock->async_recv(
                    ...
                    [sock, &buf](const libcpp::tcp_socket::err_t& err,
                        ...
                        ASSERT_EQ(err.failed(), false);
                        ASSERT_EQ(sz == 5, true);
                        ASSERT_EQ(std::string(reinterpret_cast<char*>(buf),
                                              5) == "hello",
                                  true);
                        delete sock;
                    });
...
TEST(tcp_socket, recv)
{
    ...
    ASSERT_EQ(sock.recv(buf, 1024) == 6, true);
    ASSERT_EQ(
        std::string(reinterpret_cast<char*>(buf), 5) == std::string("hello"),
        true);
    ASSERT_EQ(sock.recv(buf, 1024) == 6, true);
    ASSERT_EQ(
        std::string(reinterpret_cast<char*>(buf), 5) == std::string("harry"),
        true);
    sock.close();
...
TEST(tcp_socket, async_recv)
{
    std::thread t([]() {
        ...
        li.async_accept(
            10091,
            [&buf, &nrecved, &sock_ptr](const libcpp::tcp_listener::err_t& err,
                                        libcpp::tcp_socket* sock) {
                ASSERT_EQ(err.failed(), false);
                ASSERT_EQ(sock->is_connected(), true);
                sock_ptr = sock;
