...
TEST(clickhouse, create_table)
{
    ...
    int count = 0;
    cli.Select("SELECT count(*) FROM create_table_test limit 100", [&](const libcpp::ck::block& block){
        count++;
    });
...
TEST(clickhouse, select)
{
    ...
    cli.Select("SELECT id, name, score, dt FROM select_test WHERE id = 1", [](const libcpp::ck::block& block){
