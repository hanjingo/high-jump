...
TEST_F(CpuTest, cpu_core_num_basic)
{
    ...
    EXPECT_LE(core_count, 1024) << "CPU core count should be reasonable (≤ 1024)";
    
    for (int i = 0; i < 5; ++i) {
        EXPECT_EQ(cpu_core_num(), core_count) << "cpu_core_num should return consistent results";
    }
}
...
TEST_F(CpuTest, cpu_core_list_functionality)
{
    ...
    EXPECT_EQ(list_len, expected_count) << "Core list length should match cpu_core_num()";
    ...
    std::set<unsigned int> unique_cores(core_list.begin(), core_list.begin() + list_len);
...
TEST_F(CpuTest, cpu_core_bind_functionality)
{
    ...
    if (!any_bind_success) {
        std::cout << "Warning: CPU binding failed (may require administrator privileges)" << std::endl;
    }
    
#elif defined(__APPLE__)
    std::cout << "Testing CPU binding on macOS (should return false)..." << std::endl;
    
    for (unsigned int i = 0; i < std::min(2u, list_len); ++i) {
        bool bind_result = cpu_core_bind(core_list[i]);
        EXPECT_FALSE(bind_result) << "macOS should not support CPU binding";
        std::cout << "Bind to core " << core_list[i] << ": " 
                  << (bind_result ? "Success" : "Failed (expected)") << std::endl;
    }
#endif
}
...
TEST_F(CpuTest, cpu_id_functionality)
{
    ...
    std::cout << "CPU ID readings: " << cpu_id_1 << ", " << cpu_id_2 << ", " << cpu_id_3 << std::endl;
    ...
    std::cout << "Collected " << unique_ids.size() << " unique CPU IDs in 100 samples" << std::endl;
    
    EXPECT_GE(unique_ids.size(), 1) << "Should observe at least one CPU ID";
    EXPECT_LE(unique_ids.size(), cpu_core_num()) << "Unique CPU IDs should not exceed core count";
}
...
TEST_F(CpuTest, cpu_id_multithreaded)
{
    ...
    std::cout << "Starting " << min << " threads to test CPU ID..." << std::endl;
    ...
    for (int t = 0; t < min; ++t) {
        ...
        EXPECT_GE(thread_cpu_ids[t].size(), 1) << "Each thread should see at least one CPU ID";
    }
    
    std::cout << "Total unique CPU IDs observed across all threads: " << all_observed_cpus.size() << std::endl;
}
...
TEST_F(CpuTest, cpu_pause_functionality)
{
    ...
    std::cout << "1000 cpu_pause() calls took " << duration.count() << " microseconds" << std::endl;
...
TEST_F(CpuTest, cpu_nop_functionality)
{
    ...
    std::cout << "10000 cpu_nop() calls took " << duration.count() << " microseconds" << std::endl;
...
TEST_F(CpuTest, cpu_delay_functionality)
{
    ...
    std::cout << "Testing cpu_delay() with different cycle counts..." << std::endl;
    ...
    for (uint64_t cycles : test_cycles) {
        ...
        std::cout << "cpu_delay(" << cycles << ") took " << duration.count() << " microseconds" << std::endl;
...
TEST_F(CpuTest, cpu_delay_precision)
{
    ...
    for (int i = 0; i < num_tests; ++i) {
        ...
        std::cout << "Test " << (i + 1) << ": " << duration.count() << " microseconds" << std::endl;
    }
    ...
    std::cout << "Average: " << avg << " μs, Standard deviation: " << std_dev << " μs" << std::endl;
...
TEST_F(CpuTest, cpu_boundary_conditions)
{
    ...
    cpu_delay(0);
    auto end = std::chrono::high_resolution_clock::now();
    auto zero_delay = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
    
    std::cout << "cpu_delay(0) took " << zero_delay.count() << " microseconds" << std::endl;
    EXPECT_LT(zero_delay.count(), 1000) << "Zero delay should be very fast";
    
    start = std::chrono::high_resolution_clock::now();
    cpu_delay(10000000); // 10M cycles
    end = std::chrono::high_resolution_clock::now();
    auto large_delay = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    
    std::cout << "cpu_delay(10000000) took " << large_delay.count() << " milliseconds" << std::endl;
    EXPECT_GT(large_delay.count(), 0) << "Large delay should be measurable";
    EXPECT_LT(large_delay.count(), 10000) << "Large delay should not be excessive";
...
TEST(cpu, cpu_core_bind)
{
    ...
    for (unsigned int i = 0; i < test_count; ++i) {
        ...
        std::cout << "Bind to core " << buf[i] << ": " << (result ? "Success" : "Failed") << std::endl;
    }
    
    if (!any_success) {
        std::cout << "Warning: All CPU bindings failed (may require administrator privileges)" << std::endl;
    }
#endif
}
...
TEST_F(CpuTest, cpu_cache_flush_functionality)
{
    ...
    EXPECT_GE(duration.count(), 0) << "Cache flush should execute without error";
    EXPECT_LT(duration.count(), 10000) << "Cache flush should not take too long";
...
TEST_F(CpuTest, cpu_prefetch_functionality)
{
    ...
    std::cout << "Testing cpu_prefetch_read() and cpu_prefetch_write()..." << std::endl;
    ...
    auto no_prefetch_duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
    
    std::cout << "Array sum with prefetch: " << sum << " (took " << prefetch_duration.count() << " μs)" << std::endl;
    std::cout << "Array sum without prefetch: " << sum2 << " (took " << no_prefetch_duration.count() << " μs)" << std::endl;
    ...
    auto write_prefetch_duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
    
    std::cout << "Array write with prefetch took " << write_prefetch_duration.count() << " μs" << std::endl;
...
TEST_F(CpuTest, cpu_tsc_functionality)
{
    ...
    std::cout << "TSC readings: " << tsc1 << ", " << tsc2 << ", " << tsc3 << std::endl;
    ...
    if (all_different) {
        std::cout << "TSC shows high precision (all values increasing)" << std::endl;
    } else {
        std::cout << "TSC shows limited precision (some values equal)" << std::endl;
    }
    ...
    std::cout << "10000 TSC reads took " << duration.count() << " microseconds" << std::endl;
...
TEST_F(CpuTest, cpu_tscp_functionality)
{
    ...
    std::cout << "TSC vs TSCP comparison: TSC=" << tsc << ", TSCP=" << tscp << std::endl;
    
    if (tsc > 0 && tscp > 0) {
        uint64_t diff = (tsc > tscp) ? (tsc - tscp) : (tscp - tsc);
        double max = (tsc > tscp) ? tsc : tscp;
        double relative_diff = (double)diff / max;
        std::cout << "Relative difference between TSC and TSCP: " << (relative_diff * 100) << "%" << std::endl;
    }
}
...
TEST_F(CpuTest, cpu_pmu_cycle_counter_functionality)
{
    ...
    std::cout << "PMU cycle counter readings: " << pmu1 << ", " << pmu2 << ", " << pmu3 << std::endl;
    ...
    std::cout << "Is true cycle counter: " << (is_true_cycle_counter ? "Yes" : "No") << std::endl;
    ...
    std::cout << "  Wall time: " << wall_time.count() << " microseconds" << std::endl;
    ...
    if (wall_time.count() > 0) {
        ...
        std::cout << "  Estimated cycles per microsecond: " << cycles_per_microsecond << std::endl;
        
        if (is_true_cycle_counter) {
            EXPECT_GT(cycles_per_microsecond, 100) << "CPU cycle rate seems too low for " << counter_type;
            EXPECT_LT(cycles_per_microsecond, 10000) << "CPU cycle rate seems too high for " << counter_type;
        } else {
            std::cout << "  Note: This is not a true CPU cycle counter, so frequency checks are relaxed" << std::endl;
            EXPECT_GT(cycles_per_microsecond, 0.1) << "Counter rate should be positive for " << counter_type;
            EXPECT_LT(cycles_per_microsecond, 100000) << "Counter rate seems unreasonably high for " << counter_type;
            
            if (counter_type.find("QueryPerformanceCounter") != std::string::npos) {
                std::cout << "  QueryPerformanceCounter frequency appears to be: " 
                          << cycles_per_microsecond << " counts/μs" << std::endl;
                EXPECT_GT(cycles_per_microsecond, 0.1) << "QPC should have reasonable frequency";
                EXPECT_LT(cycles_per_microsecond, 100) << "QPC frequency should be moderate";
            }
        }
    }
}
...
TEST_F(CpuTest, cpu_counter_frequency_detection)
{
    ...
    std::cout << "Frequency detection results (over " << wall_duration.count() << " μs):" << std::endl;
    ...
    if (wall_duration.count() > 0) {
        ...
        if (tsc_freq_mhz > 100 && pmu_freq_mhz > 100) {
            ...
            std::cout << "  TSC/PMU frequency ratio: " << freq_ratio << std::endl;
            if (freq_ratio > 0.5 && freq_ratio < 2.0) {
                std::cout << "  Both counters appear to be CPU cycle counters" << std::endl;
            }
        }
        
#if defined(_WIN32) || defined(_WIN64) && (!defined(_M_IX86) && !defined(_M_X64))
        if (pmu_freq_mhz < 100) {
            std::cout << "  PMU appears to be QueryPerformanceCounter (low frequency)" << std::endl;
        }
#endif
    }
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
        if (duration.count() > 0) {
            tsc_frequencies.push_back((double)(tsc_e - tsc_s) / duration.count());
            pmu_frequencies.push_back((double)(pmu_e - pmu_s) / duration.count());
        }
    }
    ...
    if (!tsc_frequencies.empty()) {
        ...
        std::cout << "  TSC average frequency: " << tsc_avg << " MHz" << std::endl;
        std::cout << "  PMU average frequency: " << pmu_avg << " MHz" << std::endl;
        
        double tsc_var = 0, pmu_var = 0;
        for (size_t i = 0; i < tsc_frequencies.size(); ++i) {
            tsc_var += (tsc_frequencies[i] - tsc_avg) * (tsc_frequencies[i] - tsc_avg);
            pmu_var += (pmu_frequencies[i] - pmu_avg) * (pmu_frequencies[i] - pmu_avg);
        }
        
        if (tsc_avg > 0) {
            double tsc_cv = std::sqrt(tsc_var / tsc_frequencies.size()) / tsc_avg;
            std::cout << "  TSC frequency stability (CV): " << (tsc_cv * 100) << "%" << std::endl;
            EXPECT_LT(tsc_cv, 0.1) << "TSC frequency should be stable";
        }
        
        if (pmu_avg > 0) {
            double pmu_cv = std::sqrt(pmu_var / pmu_frequencies.size()) / pmu_avg;
            std::cout << "  PMU frequency stability (CV): " << (pmu_cv * 100) << "%" << std::endl;
