...
protected:
    void SetUp() override {
        ...
        ASSERT_EQ(result, DISK_SUCCESS) << "Failed to initialize disk subsystem: " 
...
TEST_F(DiskTest, initialization_and_cleanup)
{
    ...
    EXPECT_EQ(disk_init(), DISK_SUCCESS) << "Multiple initialization should be safe";
...
TEST_F(DiskTest, error_string_conversion)
{
    ...
    const disk_error_t test_errors[] = {
        DISK_SUCCESS,
    ...
    for (size_t i = 0; i < sizeof(test_errors) / sizeof(test_errors[0]); i++) {
        ...
        EXPECT_NE(error_str, nullptr) << "Error string should not be NULL for error code " << test_errors[i];
        EXPECT_GT(strlen(error_str), 0) << "Error string should not be empty for error code " << test_errors[i];
        
        std::cout << "Error " << test_errors[i] << ": " << error_str << std::endl;
    }
    
    // Test unknown error code
    const char* unknown_error = disk_get_error_string((disk_error_t)-999);
    EXPECT_STREQ(unknown_error, "Unknown error") << "Unknown error code should return 'Unknown error'";
...
TEST_F(DiskTest, disk_type_string_conversion)
{
    ...
    for (size_t i = 0; i < sizeof(test_types) / sizeof(test_types[0]); i++) {
        ...
        EXPECT_NE(type_str, nullptr) << "Disk type string should not be NULL";
        EXPECT_GT(strlen(type_str), 0) << "Disk type string should not be empty";
        
        std::cout << "Disk type " << test_types[i] << ": " << type_str << std::endl;
    }
...
TEST_F(DiskTest, filesystem_type_string_conversion)
{
    ...
    for (size_t i = 0; i < sizeof(test_filesystems) / sizeof(test_filesystems[0]); i++) {
        const char* fs_str = filesystem_type_to_string(test_filesystems[i]);
        EXPECT_NE(fs_str, nullptr) << "Filesystem type string should not be NULL";
        EXPECT_GT(strlen(fs_str), 0) << "Filesystem type string should not be empty";
        
        std::cout << "Filesystem " << test_filesystems[i] << ": " << fs_str << std::endl;
    }
    ...
    std::cout << "Filesystem type string conversion test completed" << std::endl;
}
...
TEST_F(DiskTest, size_formatting)
{
    ...
    for (size_t i = 0; i < sizeof(test_sizes) / sizeof(test_sizes[0]); i++) {
        disk_error_t result = disk_format_size(test_sizes[i], buffer, sizeof(buffer));
        EXPECT_EQ(result, DISK_SUCCESS) << "Size formatting should succeed for " << test_sizes[i];
        EXPECT_GT(strlen(buffer), 0) << "Formatted size should not be empty";
        
        std::cout << "Size " << test_sizes[i] << " bytes: " << buffer << std::endl;
    }
    ...
    char small_buffer[5];
    disk_error_t result = disk_format_size(1024ULL * 1024 * 1024, small_buffer, sizeof(small_buffer));
    EXPECT_EQ(result, DISK_ERROR_INSUFFICIENT_BUFFER) << "Should detect insufficient buffer";
...
TEST_F(DiskTest, disk_count_retrieval)
{
    ...
    if (result == DISK_ERROR_NOT_SUPPORTED) {
        std::cout << "Disk enumeration not supported on this platform" << std::endl;
...
TEST_F(DiskTest, disk_enumeration)
{
    ...
    EXPECT_LE(actual_count, count) << "Actual count should not exceed requested count";
    
    std::cout << "Successfully enumerated " << actual_count << " disks:" << std::endl;
    
    for (uint32_t i = 0; i < actual_count; i++) {
        const disk_info_t& disk = disks[i];
        
        // Validate disk information
        EXPECT_GT(strlen(disk.device_name), 0) << "Device name should not be empty for disk " << i;
        EXPECT_GE(disk.type, DISK_TYPE_UNKNOWN) << "Disk type should be valid for disk " << i;
        EXPECT_LE(disk.type, DISK_TYPE_RAM) << "Disk type should be valid for disk " << i;
        
        std::cout << "  Disk " << i << ":" << std::endl;
        std::cout << "    Device: " << disk.device_name << std::endl;
        std::cout << "    Model: " << disk.model << std::endl;
        std::cout << "    Type: " << disk_type_to_string(disk.type) << std::endl;
        ...
        std::cout << "    Sector size: " << disk.sector_size << " bytes" << std::endl;
        std::cout << "    Removable: " << (disk.removable ? "Yes" : "No") << std::endl;
        std::cout << "    Read-only: " << (disk.read_only ? "Yes" : "No") << std::endl;
        ...
        if (disk.temperature >= 0) {
            std::cout << "    Temperature: " << disk.temperature << "°C" << std::endl;
        }
    }
    
    // Test invalid parameters
    EXPECT_EQ(disk_enumerate(nullptr, count, &actual_count), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_enumerate(disks.data(), count, nullptr), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_enumerate(disks.data(), 0, &actual_count), DISK_ERROR_INVALID_PARAMETER);
...
TEST_F(DiskTest, disk_info_retrieval)
{
    ...
    if (actual_count > 0) {
        ...
        if (result == DISK_SUCCESS) {
            std::cout << "Successfully retrieved info for disk: " << disks[0].device_name << std::endl;
            
            // Validate the information
            EXPECT_STREQ(info.device_name, disks[0].device_name) << "Device name should match";
            EXPECT_GE(info.type, DISK_TYPE_UNKNOWN) << "Disk type should be valid";
            EXPECT_LE(info.type, DISK_TYPE_RAM) << "Disk type should be valid";
            
            if (info.sector_size > 0) {
                EXPECT_GE(info.sector_size, 512) << "Sector size should be at least 512 bytes";
                EXPECT_LE(info.sector_size, 4096) << "Sector size should be reasonable";
            }
        } else {
            std::cout << "Could not retrieve detailed info for disk (may require elevated privileges)" << std::endl;
        }
    }
    
    // Test invalid parameters
    disk_info_t dummy_info;
    EXPECT_EQ(disk_get_info(nullptr, &dummy_info), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_get_info("valid_name", nullptr), DISK_ERROR_INVALID_PARAMETER);
...
TEST_F(DiskTest, disk_readiness_check)
{
    ...
    if (actual_count > 0) {
        ...
        if (result == DISK_SUCCESS) {
            std::cout << "Disk " << disks[0].device_name << " ready status: " 
                     << (ready ? "Ready" : "Not ready") << std::endl;
            
            // Most system disks should be ready
            if (!disks[0].removable) {
                EXPECT_TRUE(ready) << "Non-removable disk should typically be ready";
            }
        } else if (result == DISK_ERROR_NOT_SUPPORTED) {
            std::cout << "Disk readiness check not supported on this platform" << std::endl;
        } else {
            std::cout << "Readiness check failed: " << disk_get_error_string(result) << std::endl;
        }
    }
    
    // Test invalid parameters
    bool dummy_ready;
    EXPECT_EQ(disk_is_ready(nullptr, &dummy_ready), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_is_ready("valid_name", nullptr), DISK_ERROR_INVALID_PARAMETER);
...
TEST_F(DiskTest, partition_info_retrieval)
{
    ...
    if (result == DISK_SUCCESS) {
        std::cout << "Successfully retrieved partition information:" << std::endl;
        ...
        std::cout << "  Filesystem: " << filesystem_type_to_string(info.filesystem) << std::endl;
        
        if (info.total_size > 0) {
            char total_str[64], used_str[64], avail_str[64];
            
            if (disk_format_size(info.total_size, total_str, sizeof(total_str)) == DISK_SUCCESS &&
                disk_format_size(info.used_size, used_str, sizeof(used_str)) == DISK_SUCCESS &&
                disk_format_size(info.available_size, avail_str, sizeof(avail_str)) == DISK_SUCCESS) {
                ...
                // Basic sanity checks
                EXPECT_GT(info.total_size, 0) << "Total size should be positive";
                EXPECT_LE(info.used_size, info.total_size) << "Used size should not exceed total";
                EXPECT_LE(info.available_size, info.total_size) << "Available size should not exceed total";
            }
        }
        
        std::cout << "  Read-only: " << (info.read_only ? "Yes" : "No") << std::endl;
        std::cout << "  Bootable: " << (info.bootable ? "Yes" : "No") << std::endl;
        
    } else if (result == DISK_ERROR_NOT_SUPPORTED) {
        std::cout << "Partition information not supported on this platform" << std::endl;
        GTEST_SKIP() << "Platform does not support partition information";
    } else {
        std::cout << "Could not retrieve partition information: " << disk_get_error_string(result) << std::endl;
        // This might be expected on some systems due to permissions
    }
    
    // Test invalid parameters
    partition_info_t dummy_info;
    EXPECT_EQ(disk_get_partition_by_mount(nullptr, &dummy_info), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_get_partition_by_mount("/", nullptr), DISK_ERROR_INVALID_PARAMETER);
...
TEST_F(DiskTest, disk_performance_testing)
{
    ...
    if (actual_count > 0) {
        ...
        result = disk_test_read_speed(disks[0].device_name, 1, &read_speed); // 1 MB test
        ...
        if (result == DISK_SUCCESS) {
            ...
            std::cout << "Read speed test not available (may require elevated privileges)" << std::endl;
        }
    }
    
    // Test invalid parameters
    double dummy_speed;
    EXPECT_EQ(disk_test_read_speed(nullptr, 1, &dummy_speed), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_test_read_speed("valid_name", 0, &dummy_speed), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_test_read_speed("valid_name", 1, nullptr), DISK_ERROR_INVALID_PARAMETER);
...
TEST_F(DiskTest, parameter_validation)
{
    ...
    std::cout << "Testing parameter validation across all functions..." << std::endl;
    ...
    EXPECT_EQ(disk_get_count(nullptr), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_enumerate(nullptr, 1, &dummy_uint32), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_get_info(nullptr, &dummy_disk_info), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_get_info("test", nullptr), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_get_partition_by_mount(nullptr, &dummy_partition_info), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_get_partition_by_mount("/", nullptr), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_is_ready(nullptr, &dummy_bool), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_is_ready("test", nullptr), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_test_read_speed(nullptr, 1, &dummy_double), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_test_read_speed("test", 0, &dummy_double), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_test_read_speed("test", 1, nullptr), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_format_size(1024, nullptr, 256), DISK_ERROR_INVALID_PARAMETER);
    EXPECT_EQ(disk_format_size(1024, dummy_buffer, 0), DISK_ERROR_INVALID_PARAMETER);
...
TEST_F(DiskTest, comprehensive_disk_system_test)
{
    ...
    for (uint32_t i = 0; i < actual_count; i++) {
        const disk_info_t& disk = disks[i];
        std::cout << "\nTesting disk " << i << ": " << disk.device_name << std::endl;
        
        // Test readiness
        bool ready = false;
        result = disk_is_ready(disk.device_name, &ready);
        if (result == DISK_SUCCESS) {
            std::cout << "  Readiness: " << (ready ? "Ready" : "Not ready") << std::endl;
        }
    ...
    std::cout << "\nComprehensive disk system test completed successfully" << std::endl;
}
