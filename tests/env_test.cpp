...
protected:
    ...
    bool IsPositiveValue(int64_t value) {
        return value > 0;
    }
...
TEST_F(EnvTest, MemoryConfiguration) {
    ...
    EXPECT_TRUE(page_size == 4096 || page_size == 8192 || 
...
TEST_F(EnvTest, TimeConfiguration) {
    ...
    if (IsValidValue(boot_time)) {
        ...
        if (boot_time > 631152000) { // 1990-01-01
            EXPECT_LT(boot_time, current_time + 86400); // Not more than 1 day in future
...
TEST_F(EnvTest, CompilerFeatures) {
    ...
    std::cout << "QT Environment: " << LIBCPP_QT_ENVIRONMENT << std::endl;

    // Test branch prediction macros (just ensure they compile)
    int x = 1;
    if (LIKELY(x == 1)) {
        EXPECT_TRUE(true);
    }
...
TEST_F(EnvTest, SystemInformationDisplay) {
    ...
    if (IsValidValue(ENV_CPU_FREQUENCY)) {
        std::cout << "CPU Frequency: " << ENV_CPU_FREQUENCY << " MHz" << std::endl;
    }
    
    std::cout << "Memory Total: " << ENV_MEMORY_TOTAL / (1024*1024) << " MB" << std::endl;
    ...
    std::cout << "Timezone Offset: " << ENV_TIMEZONE_OFFSET << " minutes (GMT" << 
                 (ENV_TIMEZONE_OFFSET >= 0 ? "+" : "") << ENV_TIMEZONE_OFFSET/60.0 << ")" << std::endl;
