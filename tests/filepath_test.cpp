...
{
    ASSERT_STREQ(
        hj::filepath::parent("/usr/local/src").c_str(), 
        "/usr/local"
    );
}
...
#if !defined(_WIN32)
    ASSERT_STREQ(
        hj::filepath::absolute("/usr/local/src").c_str(), 
        "/usr/local/src"
    );
#endif

    ASSERT_STREQ(
        hj::filepath::absolute("./007.txt").c_str(), 
...
TEST(file_path, relative)
{
...
#if defined(_WIN32)
    ASSERT_STREQ(
        hj::filepath::relative(pwd + "\\007.txt", pwd).c_str(),
        "007.txt"
    );
    ASSERT_STREQ(
    ...
    ASSERT_STREQ(
        hj::filepath::relative(sibling, pwd).c_str(),
        "sibling.txt"
    );
}
...
#if defined(_WIN32)
    ASSERT_STREQ(
        hj::filepath::join("/usr/local/src", "007.txt").c_str(), 
        "/usr/local/src\\007.txt"
    );
    std::vector<std::string> args{"/usr/local/src", "007.txt"};
    ASSERT_STREQ(
        hj::filepath::join(args).c_str(), 
        "/usr/local/src\\007.txt"
    );
#else
    ASSERT_STREQ(
...
{
    ASSERT_STREQ(
        hj::filepath::file_name("/usr/local/src/007.txt").c_str(), 
        "007.txt"
    );
}
...
{
    ASSERT_STREQ(
        hj::filepath::dir_name("/usr/local/src/007.txt").c_str(), 
        "src"
    );
}
...
{
    ASSERT_STREQ(
        hj::filepath::path_name("/usr/local/src/007.txt").c_str(), 
        "/usr/local/src"
    );
}
...
{
    ASSERT_STREQ(
        hj::filepath::extension("/usr/local/src/007.txt").c_str(), 
        ".txt"
    );
}
...
{
    ASSERT_STREQ(
        hj::filepath::replace_extension("/usr/local/src/007.txt", ".exe").c_str(), 
...
TEST(file_path, size)
{
    ...
    std::string f = hj::filepath::join(hj::filepath::pwd(), "file_path_size_test.txt");
...
TEST(file_path, find)
{
    std::string f = hj::filepath::join(hj::filepath::pwd(), "file_path_find_test.txt");
    if (!hj::filepath::is_exist(f))
    {
        ASSERT_EQ(hj::filepath::make_file(f), true);
    }

    ASSERT_EQ(hj::filepath::find(hj::filepath::pwd(), "file_path_find_test.txt").size() > 0, true);
}
...
TEST(file_path, make_file)
{
    ...
    ASSERT_EQ(
        (hj::filepath::is_exist(f) ? true : hj::filepath::make_file(f)), 
        true
    );
}
...
TEST(file_path, copy_dir)
{
    ...
    ASSERT_EQ(
        (hj::filepath::is_exist(from)&&(!hj::filepath::is_exist(to)) ? hj::filepath::copy_dir(from, to) : true), 
        true
    );
}
...
TEST(file_path, copy_file)
{
    ...
    ASSERT_EQ(
        (hj::filepath::is_exist(from)&&(!hj::filepath::is_exist(to)) ? hj::filepath::copy_file(from, to) : true), 
        true
    );
}
...
TEST(file_path, remove)
{
    std::string f = hj::filepath::join(hj::filepath::pwd(), "007.txt");
    ASSERT_EQ(
        (hj::filepath::is_exist(f) ? hj::filepath::remove(f) : true), 
        true
    );
}
...
TEST(file_path, rename)
{
    ...
    std::string to = hj::filepath::join(hj::filepath::pwd(), "008.txt");
    ASSERT_STREQ(
        (hj::filepath::is_exist(from)&&(!hj::filepath::is_exist(to)) ? hj::filepath::rename(from, to).c_str() : to.c_str()), 
        to.c_str()
    );
}
