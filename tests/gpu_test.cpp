...
TEST(gpu, gpu_malloc)
{
    ...
    ASSERT_TRUE(gpu_malloc(&buffer, &ctx, size)) << "Failed to allocate GPU memory";
...
TEST(gpu, gpu_malloc_read_only)
{
    ...
    size_t size = 1024 * 1024; // 1 MB
    ASSERT_TRUE(gpu_malloc_read_only(&buffer, &ctx, size)) << "Failed to allocate GPU memory";
...
TEST(gpu, gpu_malloc_write_only)
{
    ...
    size_t size = 1024 * 1024; // 1 MB
    ASSERT_TRUE(gpu_malloc_write_only(&buffer, &ctx, size)) << "Failed to allocate GPU memory";
...
TEST(gpu, gpu_malloc_pinned)
{
    ...
    size_t size = 1024 * 1024; // 1 MB
    ASSERT_TRUE(gpu_malloc_pinned(&buffer, size)) << "Failed to allocate pinned GPU memory";
...
TEST(gpu, gpu_malloc_unified)
{
    ...
    size_t size = 1024 * 1024; // 1 MB
    ASSERT_TRUE(gpu_malloc_unified(&buffer, size)) << "Failed to allocate unified GPU memory";
...
TEST(gpu, gpu_memcpy_from_device)
{
    ...
    int host_data[256] = {0};
    ASSERT_TRUE(gpu_memcpy_from_device(host_data, &src_buffer, sizeof(host_data), &ctx)) 
...
TEST(gpu, gpu_memcpy_device_to_device)
{
    ...
    ASSERT_TRUE(gpu_memcpy_device_to_device(&dst_buffer, &src_buffer, sizeof(data), &ctx)) 
        << "Failed to copy data from one device buffer to another";

    int host_data[256] = {0};
    ASSERT_TRUE(gpu_memcpy_from_device(host_data, &dst_buffer, sizeof(host_data), &ctx)) 
...
TEST(gpu, gpu_memcpy_to_device_async)
{
    ...
    ASSERT_TRUE(gpu_memcpy_to_device_async(&dst_buffer, data, sizeof(data), &ctx)) 
        << "Failed to copy data to device";
...
TEST(gpu, gpu_memcpy_from_device_async)
{
    ...
    int host_data[256] = {0};
    ASSERT_TRUE(gpu_memcpy_from_device_async(host_data, &src_buffer, sizeof(host_data), &ctx)) 
        << "Failed to copy data from device";
...
TEST(gpu, gpu_execute_kernel_1d)
{
            ...
            gpu_kernel_create(&program, "test")) 
    {
        ...
        EXPECT_TRUE(gpu_execute_kernel_1d(&program, &ctx, global_work_size, local_work_size));
...
TEST(gpu, gpu_execute_kernel_2d)
{
            ...
            gpu_kernel_create(&program, "test")) 
    {
        ...
        EXPECT_TRUE(gpu_execute_kernel_2d(&program, &ctx, global_work_size, local_work_size));
...
TEST(gpu, gpu_execute_kernel_3d)
{
            ...
            gpu_kernel_create(&program, "test")) 
    {
        ...
        EXPECT_TRUE(gpu_execute_kernel_3d(&program, &ctx, global_work_size, local_work_size));
