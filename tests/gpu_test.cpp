...
bool gpu_device_callback(cl_device_id device)
{
    char device_name[128];
    clGetDeviceInfo(device, CL_DEVICE_NAME, sizeof(device_name), device_name, NULL);
    printf("GPU Device: %s\n", device_name);
    return true; // Continue iterating through devices
};
...
TEST(gpu, gpu_device_range)
{
    gpu_device_range(gpu_device_callback);
    // The callback will print the names of all GPU devices.
    // If no devices are found, the test will fail due to the assertion in gpu_count.
    ASSERT_TRUE(true) << "GPU device range callback executed successfully";
}
