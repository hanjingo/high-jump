...
//         ASSERT_EQ(result, NIC_SUCCESS) << "Failed to initialize NIC subsystem: " 
...
//     result = nic_init();
//     EXPECT_EQ(result, NIC_SUCCESS) << "Multiple NIC initializations should be safe";
...
//     EXPECT_STREQ(success_msg, "Success") << "Success message should be correct";
    
//     const char* invalid_param_msg = nic_get_error_string(NIC_ERROR_INVALID_PARAMETER);
//     EXPECT_STREQ(invalid_param_msg, "Invalid parameter") << "Invalid parameter message should be correct";
    
//     const char* not_found_msg = nic_get_error_string(NIC_ERROR_NOT_FOUND);
//     EXPECT_STREQ(not_found_msg, "Interface not found") << "Not found message should be correct";
    
//     const char* unknown_msg = nic_get_error_string(static_cast<nic_error_t>(-999));
//     EXPECT_STREQ(unknown_msg, "Unknown error") << "Unknown error message should be correct";
...
//     EXPECT_EQ(result, NIC_ERROR_INVALID_PARAMETER) << "Should return error for null parameter";
...
//     EXPECT_LE(count, NIC_MAX_INTERFACES) << "Interface count should not exceed maximum";
...
//         std::cout << "    Type: " << nic_type_to_string(nic.type) << std::endl;
//         std::cout << "    Status: " << nic_status_to_string(nic.status) << std::endl;
...
//     EXPECT_EQ(result, NIC_ERROR_INVALID_PARAMETER) << "Should return error for null interface name";
    
//     result = nic_get_interface_info("eth0", nullptr);
//     EXPECT_EQ(result, NIC_ERROR_INVALID_PARAMETER) << "Should return error for null info pointer";
...
//         EXPECT_STREQ(info.name, first_interface_name) << "Retrieved interface name should match";
...
//     EXPECT_EQ(result, NIC_ERROR_INVALID_PARAMETER) << "Should return error for null stats pointer";
...
//         EXPECT_GE(stats.bytes_received, 0) << "Bytes received should be non-negative";
//         EXPECT_GE(stats.packets_sent, 0) << "Packets sent should be non-negative";
//         EXPECT_GE(stats.packets_received, 0) << "Packets received should be non-negative";
//     } else {
//         std::cout << "Statistics not available for interface " << first_interface_name 
...
//     nic_error_t result = nic_format_mac_address(&mac, mac_str, sizeof(mac_str));
//     EXPECT_EQ(result, NIC_SUCCESS) << "MAC address formatting should succeed";
//     EXPECT_STREQ(mac_str, "00:11:22:33:44:55") << "MAC address format should be correct";
    
//     // Test invalid parameters for MAC formatting
//     result = nic_format_mac_address(nullptr, mac_str, sizeof(mac_str));
//     EXPECT_EQ(result, NIC_ERROR_INVALID_PARAMETER) << "Should return error for null MAC address";
    
//     result = nic_format_mac_address(&mac, nullptr, sizeof(mac_str));
//     EXPECT_EQ(result, NIC_ERROR_INVALID_PARAMETER) << "Should return error for null buffer";
    
//     result = nic_format_mac_address(&mac, mac_str, 10);
//     EXPECT_EQ(result, NIC_ERROR_INVALID_PARAMETER) << "Should return error for insufficient buffer";
...
//     EXPECT_EQ(result, NIC_ERROR_INVALID_PARAMETER) << "Should return error for invalid MAC format";
    
//     result = nic_parse_mac_address(nullptr, &parsed_mac);
//     EXPECT_EQ(result, NIC_ERROR_INVALID_PARAMETER) << "Should return error for null MAC string";
    
//     result = nic_parse_mac_address("AA:BB:CC:DD:EE:FF", nullptr);
//     EXPECT_EQ(result, NIC_ERROR_INVALID_PARAMETER) << "Should return error for null MAC structure";
...
//     EXPECT_EQ(result, NIC_ERROR_INVALID_PARAMETER) << "Should return error for null is_wireless pointer";
...
//         EXPECT_EQ(result, NIC_SUCCESS) << "Wireless detection should succeed for interface " << interfaces[i].name;
        
//         std::cout << "Interface " << interfaces[i].name << " is " 
//                   << (is_wireless ? "wireless" : "not wireless") << std::endl;
        
//         // Check consistency with interface info
//         if (interfaces[i].is_wireless) {
//             EXPECT_TRUE(is_wireless) << "Interface marked as wireless should be detected as wireless";
//         }
//     }
...
//     std::cout << "Testing interface control (may require privileges)..." << std::endl;
...
//     EXPECT_NE(result, NIC_SUCCESS) << "Should return error for non-existent interface";
...
//         EXPECT_EQ(result, NIC_SUCCESS) << "Interface enumeration should succeed in iteration " << i;
//     }
...
//                     nic_info_t interfaces[NIC_MAX_INTERFACES];
//                     uint32_t actual_count = 0;
//                     result = nic_enumerate_interfaces(interfaces, NIC_MAX_INTERFACES, &actual_count);
//                     results[i] = (result == NIC_SUCCESS);
//                 }
//             } catch (...) {
//                 results[i] = false;
//             }
...
//     EXPECT_STREQ(error_msg, "Success") << "Error string conversion should work";
    
//     const char* type_str = nic_type_to_string(NIC_TYPE_ETHERNET);
//     EXPECT_STREQ(type_str, "Ethernet") << "Type string conversion should work";
