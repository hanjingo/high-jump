...
TEST_F(ram, system_memory_information)
{
    ...
    if(result == RAM_SUCCESS)
    {
        ...
        EXPECT_GT(info.total_physical, 0)
            << "Total physical memory should be greater than 0";
        EXPECT_LE(info.used_physical, info.total_physical)
            << "Used memory should not exceed total";
        EXPECT_LE(info.available_physical, info.total_physical)
            << "Available memory should not exceed total";
        EXPECT_GE(info.memory_load, 0.0)
            << "Memory load should be non-negative";
...
TEST_F(ram, ram_module_enumeration)
{
    ...
    if(result == RAM_SUCCESS)
    {
        ...
        for(uint32_t i = 0; i < actual_count; i++)
        {
            ...
            std::cout << "    Voltage: " << module.voltage << " mV"
                      << std::endl;
            std::cout << "    Slot Number: " << module.slot_number << std::endl;
            std::cout << "    ECC: " << (module.is_ecc ? "Yes" : "No")
                      << std::endl;
            std::cout << "    Registered: "
...
TEST_F(ram, large_page_allocation)
{
    ...
    if(result == RAM_SUCCESS)
    {
        ...
        {
            EXPECT_EQ(byte_ptr[i], 0x55)
                << "Large page memory content should be preserved";
        }
    ...
    for(size_t i = 0; i < num_protections; i++)
    {
        ...
        if(result == RAM_SUCCESS)
        {
            ...
            if(protections[i] & RAM_PROTECTION_WRITE)
            {
                uint8_t *byte_ptr = (uint8_t *) ptr;
                byte_ptr[0]       = 0x42;
                EXPECT_EQ(byte_ptr[0], 0x42)
                    << "Write should succeed with write protection";
            }
        } else
        {
            std::cout << "Protection mode " << protections[i]
                      << " not supported: " << result << std::endl;
        }
    }
...
TEST_F(ram, memory_locking)
{
    ...
    if(result == RAM_SUCCESS)
    {
        ...
        if(result == RAM_SUCCESS)
        {
            ...
            EXPECT_EQ(byte_ptr[0], 0x33)
                << "Unlocked memory should still be accessible";
        }
    } else
    {
...
TEST_F(ram, memory_prefetching)
{
    ...
    {
        std::cout << "Memory prefetching not supported: " << result
                  << std::endl;
    }
