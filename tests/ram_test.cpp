...
TEST_F(RamTest, system_memory_information)
{
    ...
    EXPECT_EQ(result, RAM_ERR_INVALID_PARAMETER) << "Should return error for null parameter";
...
TEST_F(RamTest, ram_module_enumeration)
{
    ...
    EXPECT_EQ(result, RAM_ERR_INVALID_PARAMETER) << "Should return error for null modules array";
    
    ram_module_info_t modules[4];
    result = ram_get_modules(modules, 4, nullptr);
    EXPECT_EQ(result, RAM_ERR_INVALID_PARAMETER) << "Should return error for null actual_count";
    
    result = ram_get_modules(modules, 0, &actual_count);
    EXPECT_EQ(result, RAM_ERR_INVALID_PARAMETER) << "Should return error for zero max_modules";
...
TEST_F(RamTest, aligned_memory_allocation)
{
    ...
    EXPECT_EQ(result, RAM_ERR_INVALID_PARAMETER) << "Should return error for zero size";
    
    result = ram_allocate_aligned(1024, 0, &ptr);
    EXPECT_EQ(result, RAM_ERR_INVALID_PARAMETER) << "Should return error for zero alignment";
    
    result = ram_allocate_aligned(1024, 16, nullptr);
    EXPECT_EQ(result, RAM_ERR_INVALID_PARAMETER) << "Should return error for null pointer";
    
    result = ram_allocate_aligned(1024, 15, &ptr);
    EXPECT_EQ(result, RAM_ERR_INVALID_PARAMETER) << "Should return error for non-power-of-2 alignment";
    ...
    EXPECT_EQ(result, RAM_ERR_INVALID_PARAMETER) << "Should return error for freeing null pointer";
}
...
TEST_F(RamTest, memory_prefetching)
{
    ...
    if (result == RAM_SUCCESS) {
        ...
        std::cout << "Memory prefetching not supported: " << result << std::endl;
    }
