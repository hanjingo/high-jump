...
TEST(reflect, type_name)
{
    ...
    ASSERT_NE(hj::reflect::type_name(TestStruct()).find("TestStruct"), std::string::npos);
}
...
{
    ASSERT_EQ(hj::reflect::offset_of(&TestStruct::a), static_cast<std::size_t>(offsetof(TestStruct, a)));
    ASSERT_EQ(hj::reflect::size_of(&TestStruct::a), sizeof(((TestStruct*)0)->a));
...
TEST(reflect, is_same_type)
{
    ...
    ASSERT_TRUE(hj::reflect::is_same_type(a, int(2), int(3), int(4), int(5), int(6)));
    ASSERT_TRUE(hj::reflect::is_same_type(a, int(2), int(3), int(4), int(5), int(6), int(7)));
}
