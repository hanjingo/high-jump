...
TEST(rom, load_success)
{
    ...
    EXPECT_EQ(hj_rom_load(&_rom, test_file.c_str()), HJ_ROM_SUCCESS);
...
TEST(rom, load_error_conditions)
{
    ...
    EXPECT_EQ(hj_rom_load(nullptr, "dummy.rom"), HJ_ROM_ERR_INVALID_PARAM);
    EXPECT_EQ(hj_rom_load(&_rom, nullptr), HJ_ROM_ERR_INVALID_PARAM);

    std::string non_existent =
        (std::filesystem::current_path() / "non_existent.rom").string();
    std::filesystem::remove_all(non_existent);
    EXPECT_EQ(hj_rom_load(&_rom, non_existent.c_str()), HJ_ROM_ERR_FILE_NOT_FOUND);
    ...
    {
        EXPECT_EQ(hj_rom_load(&_rom, empty_file.c_str()), HJ_ROM_ERR_EMPTY_FILE);
        std::filesystem::remove_all(empty_file);
    }
}
...
TEST(rom, read_operations)
{
    ...
    hj_rom_t _rom;
    hj_rom_init(&_rom);
    ASSERT_EQ(hj_rom_load(&_rom, test_file.c_str()), HJ_ROM_SUCCESS);

    char   buf[32]  = {0};
    size_t out_read = 0;

    EXPECT_EQ(hj_rom_read(&_rom, 0, buf, 5, &out_read), HJ_ROM_SUCCESS);
    EXPECT_EQ(out_read, 5u);
    EXPECT_EQ(std::string(buf, 5), "HELLO");

    memset(buf, 0, sizeof(buf));
    EXPECT_EQ(hj_rom_read(&_rom, 0, buf, 0, &out_read), HJ_ROM_SUCCESS);
    EXPECT_EQ(out_read, 0u);

    EXPECT_EQ(hj_rom_read(&_rom, 100, buf, 4, &out_read), HJ_ROM_ERR_OUT_OF_BOUNDS);

    memset(buf, 0, sizeof(buf));
    size_t offset = test_content.size() - 5;
    EXPECT_EQ(hj_rom_read(&_rom, offset, buf, 20, &out_read), HJ_ROM_SUCCESS);
    EXPECT_EQ(out_read, 5u);
...
TEST(rom, read_invalid_state)
{
    ...
    EXPECT_EQ(hj_rom_read(&_rom, 0, buf, sizeof(buf), &out_read),
              HJ_ROM_ERR_NOT_LOADED);

    EXPECT_EQ(hj_rom_read(nullptr, 0, buf, sizeof(buf), &out_read),
              HJ_ROM_ERR_INVALID_PARAM);
    ...
    EXPECT_EQ(hj_rom_read(&_rom, 0, nullptr, 4, &out_read), HJ_ROM_ERR_INVALID_PARAM);
...
TEST(rom, transactional_reload)
{
    ...
    ASSERT_EQ(hj_rom_load(&_rom, file1.c_str()), HJ_ROM_SUCCESS);
    EXPECT_EQ(_rom.size, 9u);

    EXPECT_NE(hj_rom_load(&_rom, non_existent.c_str()), HJ_ROM_SUCCESS);
    EXPECT_TRUE(_rom.loaded);
    EXPECT_EQ(_rom.size, 9u);
    ...
    EXPECT_EQ(hj_rom_load(&_rom, file2.c_str()), HJ_ROM_SUCCESS);
    EXPECT_EQ(_rom.size, 19u);
    memset(buf, 0, sizeof(buf));
    EXPECT_EQ(hj_rom_read(&_rom, 0, buf, 6, &out_read), HJ_ROM_SUCCESS);
