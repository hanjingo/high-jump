...
{
    ASSERT_TRUE(hj::string_util::end_with("hello world", "world"));
    ASSERT_FALSE(hj::string_util::end_with("hello world", "worl"));
}

TEST(string_util, search)
{
    ASSERT_EQ(hj::string_util::search("hello w123orld", R"(\d+)") == std::string("123"), true);
}
...
TEST(string_util, split)
{
    ...
    auto arr2 = hj::string_util::split("broadcast,database,quote,sentinel", ",");
