...
TEST(tcp_listener, set_option)
{
    std::thread t([]() {
        ...
        li.async_accept(12000, [](const libcpp::tcp_listener::err_t& err, libcpp::tcp_socket* sock) {
...
TEST(tcp_listener, async_accept)
{
    ...
    std::thread t1([](){
        libcpp::tcp_socket::io_t io;
        libcpp::tcp_listener li{
            io,
            [](const libcpp::tcp_listener::err_t& err, libcpp::tcp_socket* sock) {
                ASSERT_EQ(err.failed(), false);
    ...
    std::thread t2([](){
        ...
        for (int i = 0; i < 2; i++)
        {
            li.async_accept(12003, [](const libcpp::tcp_listener::err_t& err, libcpp::tcp_socket* sock) {
                ASSERT_EQ(err.failed(), false);
                ASSERT_EQ(sock->is_connected(), true);
                ASSERT_EQ(sock->check_connected(), true);
                async_accept_times2++;
    ...
    libcpp::tcp_socket::io_t io;
    libcpp::tcp_socket sock{io};
    sock.async_connect("127.0.0.1", 12002, [&sock](const libcpp::tcp_socket::err_t& err){
        ASSERT_EQ(err.failed(), false);
        ASSERT_EQ(sock.is_connected(), true);
    });

    libcpp::tcp_socket sock1{io};
    sock1.async_connect("127.0.0.1", 12002, [&sock1](const libcpp::tcp_socket::err_t& err){
        ASSERT_EQ(err.failed(), false);
        ASSERT_EQ(sock1.is_connected(), true);
    });

    libcpp::tcp_socket sock2{io};
    sock2.async_connect("127.0.0.1", 12003, [&sock2](const libcpp::tcp_socket::err_t& err){
        ASSERT_EQ(err.failed(), false);
        ASSERT_EQ(sock2.is_connected(), true);
    });

    libcpp::tcp_socket sock3{io};
    sock3.async_connect("127.0.0.1", 12003, [&sock3](const libcpp::tcp_socket::err_t& err){
        ASSERT_EQ(err.failed(), false);
