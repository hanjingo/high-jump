...
TEST(tcp_listener, async_accept)
{
    ...
    libcpp::tcp_socket sock1{io};
    sock1.async_connect("127.0.0.1", 10091, [&sock1](const libcpp::tcp_socket::err_t& err){
        ASSERT_EQ(err.failed(), false);
        ASSERT_EQ(sock1.is_connected(), true);
    });

    libcpp::tcp_socket sock2{io};
    sock2.async_connect("127.0.0.1", 10092, [&sock2](const libcpp::tcp_socket::err_t& err){
        ASSERT_EQ(err.failed(), false);
        ASSERT_EQ(sock2.is_connected(), true);
    });

    libcpp::tcp_socket sock3{io};
    sock3.async_connect("127.0.0.1", 10092, [&sock3](const libcpp::tcp_socket::err_t& err){
        ASSERT_EQ(err.failed(), false);
